...
class GPT(nn.Module):
	def __init__(self,h):
		...
		if h.num_loops>0:
			loop_seg=list(range(h.loop_start,h.loop_end+1));self.phase_index_lists=[]
			flat=list(range(h.num_layers));ne0=len(flat)//2;self.phase_index_lists.append((flat[:ne0],flat[ne0:]))
			for p in range(1,h.num_loops+1):
				ai=list(range(h.loop_start))
				for _ in range(p+1):ai.extend(loop_seg)
				ai.extend(range(h.loop_end+1,h.num_layers));ne=len(ai)//2;self.phase_index_lists.append((ai[:ne],ai[ne:]))
			self.encoder_indices,self.decoder_indices=self.phase_index_lists[-1]
		else:self.encoder_indices=list(range(self.num_encoder_layers));self.decoder_indices=list(range(self.num_encoder_layers,h.num_layers));self.phase_index_lists=[(self.encoder_indices,self.decoder_indices)]
		self.num_skip_weights=max(min(len(e),len(d))for(e,d)in self.phase_index_lists);self.skip_weights=nn.Parameter(torch.ones(self.num_skip_weights,h.model_dim,dtype=torch.float32));self.skip_gates=nn.Parameter(torch.zeros(self.num_skip_weights,h.model_dim,dtype=torch.float32))if h.skip_gates_enabled else _A;self._init_weights()
	...
	def forward_logits(self,input_ids):
		...
		x0=x;skips=[];enc_iter,dec_iter=self.phase_index_lists[self.current_phase]
...
def train_model(h,device,val_data):
	...
	if h.warmup_steps>0:
		...
		if h.num_loops>0:
			for p in range(1,h.num_loops+1):
				base_model.current_phase=p;log(f"loop_warmup:phase={p} encoder:{base_model.phase_index_lists[p][0]} decoder:{base_model.phase_index_lists[p][1]}")
				for warmup_step in range(h.warmup_steps):
					step_fn(warmup_step,_D)
					if warmup_step<=5 or(warmup_step+1)%10==0 or warmup_step+1==h.warmup_steps:log(f"loop_warmup_step:phase={p} {warmup_step+1}/{h.warmup_steps}")
			base_model.current_phase=0
	...
	while _B:
		...
		if should_validate and h.num_loops>0 and base_model.current_phase<h.num_loops:
			smart_frac=training_time_ms/(h.max_wallclock_seconds*1e3) if h.max_wallclock_seconds>0 else _D
			while base_model.current_phase<h.num_loops and val_bpb<h.loop_phase_thresholds[base_model.current_phase] and smart_frac>=h.loop_phase_min_frac[base_model.current_phase]:
				base_model.current_phase+=1;log(f"phase_advance:smart step:{step} val_bpb:{val_bpb:.4f} frac:{smart_frac:.3f} phase:{base_model.current_phase} enc:{base_model.phase_index_lists[base_model.current_phase][0]} dec:{base_model.phase_index_lists[base_model.current_phase][1]}")
		if last_step:
		...
		if h.num_loops>0 and base_model.current_phase<h.num_loops:
			target_phase=base_model.current_phase
			while target_phase<h.num_loops and frac>=h.loop_phase_step_max[target_phase]:target_phase+=1
			if target_phase>base_model.current_phase:base_model.current_phase=target_phase;log(f"phase_advance:step_ceil step:{step} frac:{frac:.3f} phase:{target_phase} enc:{base_model.phase_index_lists[target_phase][0]} dec:{base_model.phase_index_lists[target_phase][1]}")
...
def train_and_eval(h,device):
	...
	if h.num_loops>0:eval_model.current_phase=h.num_loops
	...
	if h.ttt_enabled and h.sliding_window_enabled:
		...
		if h.num_loops>0:ttt_model.current_phase=h.num_loops
