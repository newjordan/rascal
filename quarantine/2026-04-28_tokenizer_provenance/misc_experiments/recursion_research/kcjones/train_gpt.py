...
def train_model(h,device,val_data):
	...
	while _B:
		...
		if should_validate and h.num_loops>0 and base_model.current_phase<h.num_loops:
			while base_model.current_phase<h.num_loops and val_bpb<h.loop_phase_thresholds[base_model.current_phase]:
				base_model.current_phase+=1;log(f"phase_advance:val_bpb step:{step} val_bpb:{val_bpb:.4f} phase:{base_model.current_phase} enc:{base_model.phase_index_lists[base_model.current_phase][0]} dec:{base_model.phase_index_lists[base_model.current_phase][1]}")
		if last_step:
