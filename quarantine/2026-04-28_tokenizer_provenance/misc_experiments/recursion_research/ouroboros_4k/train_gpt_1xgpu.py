...
class Hyperparameters:
    ...
    crawler_loop_rope_scales = tuple(
        int(x) for x in os.environ.get("CRAWLER_LOOP_ROPE_SCALES", "9,1,1").split(",") if x.strip()
    )
    ...
    use_crawler = bool(int(os.environ.get("USE_CRAWLER", "1")))
    num_flat_layers = int(os.environ.get("NUM_FLAT_LAYERS", 9))    # unique blocks, run once
    ...
    crawler_mlp_mult = float(os.environ.get("CRAWLER_MLP_MULT", 6.0))  # MLP width multiplier for crawler
    loop_phase_bpb_gates = bool(int(os.environ.get("LOOP_PHASE_BPB_GATES", "1")))
    loop_phase_thresholds = (1.5, 0.95)
    loop_phase_min_frac = (0.10, 0.30)
    loop_phase_step_max = (0.40, 0.65)
    ...
    loop_aware_gptq = bool(int(os.environ.get("LOOP_AWARE_GPTQ", "1")))
    ...
    gptq_cal_samples = int(os.environ.get("GPTQ_CAL_SAMPLES", "128"))
    gptq_cal_seq_len = int(os.environ.get("GPTQ_CAL_SEQ_LEN", "2048"))
...
class CrawlerGPT(nn.Module):
    ...
    def _run_crawler(self, x: Tensor, x0: Tensor, input_ids: Tensor, ve_cache: dict,
                     enc_outputs: list | None = None, max_loops: int | None = None) -> Tensor:
        ...
        n_loops = self.crawler_loops if max_loops is None else min(max_loops, self.crawler_loops)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        ve_cache: dict = {}
        if self.num_crawler_layers > 0 and self.current_phase > 0:
            x = self._run_crawler(x, x0, input_ids, ve_cache, enc_outputs=skips, max_loops=self.current_phase)
...
def main() -> None:
    ...
    if args.skip_train:
        ...
        if args.use_crawler:
            for p in range(1, args.crawler_loops + 1):
                base_model.current_phase = p
                log0(f"loop_warmup:phase={p}")
                for warmup_step in range(args.warmup_steps):
                    zero_grad_all()
                    for micro_step in range(grad_accum_steps):
                        if distributed:
                    ...
                    zero_grad_all()
                    if args.warmup_steps <= 20 or (warmup_step + 1) % 10 == 0 or warmup_step + 1 == args.warmup_steps:
                        log0(f"loop_warmup_step:phase={p} {warmup_step + 1}/{args.warmup_steps}")
    ...
    while True:
        ...
        if should_validate:
            ...
            if args.loop_phase_bpb_gates and args.use_crawler and base_model.current_phase < args.crawler_loops:
                smart_frac = training_time_ms / (args.max_wallclock_seconds * 1000.0) if args.max_wallclock_seconds > 0 else 1.0
                while (base_model.current_phase < args.crawler_loops
                       and val_bpb < args.loop_phase_thresholds[base_model.current_phase]
                       and smart_frac >= args.loop_phase_min_frac[base_model.current_phase]):
                    base_model.current_phase += 1
                    log0(f"phase_advance:smart step:{step} val_bpb:{val_bpb:.4f} frac:{smart_frac:.3f} phase:{base_model.current_phase}")
            torch.cuda.synchronize()
            t0 = time.perf_counter()
        if last_step:
        ...
        if args.use_crawler and base_model.current_phase < args.crawler_loops:
            train_frac = elapsed_ms / max_wallclock_ms if max_wallclock_ms else step / max(args.iterations, 1)
            target_phase = base_model.current_phase
            while target_phase < args.crawler_loops and train_frac >= args.loop_phase_step_max[target_phase]:
                target_phase += 1
            if target_phase > base_model.current_phase:
                base_model.current_phase = target_phase
                log0(f"phase_advance:step_ceil step:{step} frac:{train_frac:.3f} phase:{target_phase}")
        zero_grad_all()
    ...
    eval_model.current_phase = base_model.current_phase if args.use_crawler else 0
