...
class Hyperparameters:
    data_path = os.environ.get("DATA_PATH", "/workspace/data/datasets/fineweb10B_sp8192")
    train_files = os.path.join(data_path, "fineweb_train_*.bin")
    val_files = os.path.join(data_path, "fineweb_val_*.bin")
    tokenizer_path = os.environ.get("TOKENIZER_PATH", "/workspace/data/tokenizers/fineweb_8192_bpe.model")
    ...
    loop_phase_bpb_gates = bool(int(os.environ.get("LOOP_PHASE_BPB_GATES", "0")))
    loop_phase_thresholds = (3.0, 1.5)
    ...
    while True:
        ...
        if should_validate:
            ...
            if args.loop_phase_bpb_gates and args.use_crawler and base_model.current_phase < args.crawler_loops:
                while base_model.current_phase < args.crawler_loops and val_bpb < args.loop_phase_thresholds[base_model.current_phase]:
                    base_model.current_phase += 1
                    log0(f"phase_advance:val_bpb step:{step} val_bpb:{val_bpb:.4f} phase:{base_model.current_phase}")
            torch.cuda.synchronize()
            t0 = time.perf_counter()
        if last_step:
