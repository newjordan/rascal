...
Mikey 1.1 (eval-fix) plus score-first test-time training (TTT) on the
...
|---|---|---|
| Mikey 1.1 | artifact-sliding only | re-establish the corrected baseline; fix PR #1848 |
| Mikey 1.2 (this) | artifact-sliding + TTT | upside test on top of 1.1 baseline |

## Pending

`val_bpb` is null until the 8xH100 run lands. Run Mikey 1.1 first to lock the
baseline, then 1.2 to measure the TTT delta.

```
torchrun --standalone --nproc_per_node=8 Mikey_1.2/train_gpt_8xgpu.py
```
