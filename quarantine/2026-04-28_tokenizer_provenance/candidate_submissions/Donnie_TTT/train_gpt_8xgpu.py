...
os.environ.setdefault("RUN_ID", "donnie_ttt_8x_seed444_" + time.strftime("%Y%m%d_%H%M%S"))
...
# --- Score-first test-time training (TTT). PR #1493 precedent. ---
# Each chunk is scored under no_grad first (commits bytes), THEN SGD trains on the
# same chunk, THEN the loader advances. Every token is scored before it is ever
# trained on, satisfying LEADERBOARD_RULES.md. The artifact bytes are unchanged;
# TTT only modifies the in-memory deserialized eval_model.
def eval_val_ttt(
    args: Hyperparameters,
    base_model: nn.Module,
    rank: int,
...
def main() -> None:
    ...
    if world_size != 8:
        raise ValueError(
            f"Donnie_TTT 8x requires WORLD_SIZE=8, got {world_size}. "
            "Launch with: torchrun --standalone --nproc_per_node=8 Donnie_TTT/train_gpt_8xgpu.py"
        )
    ...
    log0("condition_id:donnie_ttt_8x_seed444")
    log0("run_label:donnie source_record:raphe_1p1_recipe axis:11L+layer_loop+brotli+mixed_int")
    log0("changed_fields:num_layers (10->11), num_loops (0->2), loop_start (-,2), loop_end (-,4), enable_looping_at (-,0.35), eval_target (base_model->eval_model_artifact_sliding)")
