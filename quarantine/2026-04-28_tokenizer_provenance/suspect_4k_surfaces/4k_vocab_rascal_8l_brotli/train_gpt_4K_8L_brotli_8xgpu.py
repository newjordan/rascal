...
os.environ.setdefault("RUN_ID", "rascal_4k_8L_brotli_8x_seed444_" + time.strftime("%Y%m%d_%H%M%S"))
...
def main() -> None:
    ...
    if world_size != 8:
        raise ValueError(
            f"Rascal 4k 8L brotli 8x requires WORLD_SIZE=8, got {world_size}. "
            "Launch with: torchrun --standalone --nproc_per_node=8 4k_vocab_rascal_8l_brotli/train_gpt_4K_8L_brotli_8xgpu.py"
        )
    ...
    log0("condition_id:rascal_4k_8L_brotli_8x_seed444")
    log0("run_label:salvage_attempt source_record:rascal_4k_8x_seed444_run20260427 axis:depth_8L_size_shrink+brotli")
    log0("changed_fields:num_layers (11->8), compression (zstd->brotli+bshf)")
