...
def main() -> None:
    ...
    if world_size not in (2, 8):
        raise ValueError(
            f"Mikey 1.1 SP5200 mechanics runner requires WORLD_SIZE=2 or 8, got {world_size}. "
            "Launch with: torchrun --standalone --nproc_per_node=2 run.py for the 2x scout."
        )
    ...
    log0("condition_id:2026-04-29_mikey_1p1_sp5200_2x")
    log0("run_label:new_experiment source_record:Mikey_1.1 train_gpt_8xgpu.py axis:sp5200_foundation")
    log0("changed_fields:vocab/data/tokenizer sp4096->sp5200, gpu_count 8->2 mechanics_proxy, world_size 8->2, wallclock 600->2400, coprime_shards_per_batch 1->80")
    log0("expected_metric:final_sliding_window_exact comparator:mikey_1p1_sp4096_and_sp5200_signal_only")
