...
os.environ.setdefault("RUN_ID", "mikey_III_8k_Loop_2x_seed444_" + time.strftime("%Y%m%d_%H%M%S"))
...
class Hyperparameters:
    data_path = os.environ.get("DATA_PATH", "./data/datasets/fineweb10B_sp5200")
    train_files = os.path.join(data_path, "fineweb_train_*.bin")
    val_files = os.path.join(data_path, "fineweb_val_*.bin")
    tokenizer_path = os.environ.get("TOKENIZER_PATH", "./data/tokenizers/fineweb_5200_bpe.model")
    ...
    vocab_size = int(os.environ.get("VOCAB_SIZE", 5200))
    ...
    coprime_shards_per_batch = int(os.environ.get("COPRIME_SHARDS_PER_BATCH", 80))
...
class GPT(nn.Module):
    def __init__(
        ...
        if num_loops > 0:
            if loop_start < 0 or loop_end >= num_layers or loop_start > loop_end:
                raise ValueError(f"Invalid loop range: start={loop_start} end={loop_end} num_layers={num_layers}")
        ...
        if parallel_residual_start >= 0:
            for i in range(max(0, parallel_residual_start), num_layers):
...
def main() -> None:
    ...
    if world_size not in (2, 8):
        raise ValueError(
            f"{os.environ.get('CONDITION_ID', 'Mikey 1.1')} supports WORLD_SIZE=8 exact or "
            f"WORLD_SIZE=2 mechanics proxy, got {world_size}. "
            "Launch this leg with: torchrun --standalone --nproc_per_node=2 run.py"
        )
    ...
    log0("condition_id:2026-04-29_mikey_III_sp5200_Loop_2x")
    log0("run_label:new_experiment source_script:quarantine/2026-04-28_tokenizer_provenance/candidate_submissions/Mikey_1.1/train_gpt_8xgpu.py source_script_sha256:a79045bff7058708f60b450a845c74f09a517f08b3794cc4b1b6c0b0edf077b4")
    log0("loop_source:PR1493 https://github.com/openai/parameter-golf/pull/1493 source_head:857de4755ef003134e4d3130ca792ff54861c577 wrapped_train_gpt_sha1:bc965bee09fa550e6fc6932597611ddfb8cd92ee")
    log0("changed_fields:vocab 4096->5200, tokenizer/data sp4096->sp5200, add PR1493 depth recurrence, qk_gain env override if set, coprime_shards_per_batch 1->80, gpus 8->2 mechanics_proxy")
    log0("expected_metric:final_sliding_window_exact comparator:mikey_1p1_clean_and_sp5200_loop_signal_only")
    ...
    log0(f"condition:PARALLEL_RESIDUAL_START={args.parallel_residual_start}")
    ...
    parallel_layers = [i for i, b in enumerate(base_model.blocks) if b.parallel]
    log0(f"parallel_residual:start:{args.parallel_residual_start} active_layers:{parallel_layers}")
    log0(f"layer_loop:configured num_loops:{args.num_loops} encoder:{base_model.encoder_indices} decoder:{base_model.decoder_indices}")
    ...
    if args.num_loops > 0:
        ...
        log0(f"eval_layer_loop:enabled encoder:{eval_model.encoder_indices} decoder:{eval_model.decoder_indices}")
    torch.cuda.synchronize()
