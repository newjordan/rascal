...
os.environ.setdefault("RUN_ID", "mikey_1p1_8x_seed444_" + time.strftime("%Y%m%d_%H%M%S"))
...
KERNEL_SMOKE_ATTENTION_MODE = os.environ.get("KERNEL_SMOKE_ATTENTION_MODE", "").strip().lower()
if KERNEL_SMOKE_ATTENTION_MODE:
    os.environ.setdefault("WHALE_BWD_VARIANT", "fused_delta_tma")
    os.environ.setdefault("WHALE_FUSED_DELTA_T_MAX", "3072")
    os.environ.setdefault("WHALE_BWD_Q_CONFIG", "128,64,8,5")
    os.environ.setdefault("WHALE_BWD_Q_MAXNREG", "1984")
    os.environ.setdefault("WHALE_BWD_KV_TMA_CONFIG", "128,64,8,3")
    os.environ.setdefault("WHALE_BWD_KV_TMA_MAXNREG", "256")
    try:
        from vault.whale_kernel_triton import custom_whale_attn_fwd, whale_fwd_fa3_bwd
    except ImportError as exc:
        raise RuntimeError("KERNEL_SMOKE_ATTENTION_MODE requires vault/whale_kernel_triton.py") from exc
    if KERNEL_SMOKE_ATTENTION_MODE == "whale_native":
        flash_attn_3_func = custom_whale_attn_fwd
    elif KERNEL_SMOKE_ATTENTION_MODE == "whale_fa3_bwd":
        flash_attn_3_func = whale_fwd_fa3_bwd
    elif KERNEL_SMOKE_ATTENTION_MODE == "fa3":
        pass
    else:
        raise ValueError(
            "KERNEL_SMOKE_ATTENTION_MODE must be one of: whale_native, whale_fa3_bwd, fa3"
        )
...
def main() -> None:
    ...
    allow_kernel_smoke_world_size = os.environ.get("KERNEL_SMOKE_ALLOW_WORLD_SIZE", "0") == "1"
    if world_size != 8 and not allow_kernel_smoke_world_size:
        raise ValueError(
            f"Mikey 1.1 8x requires WORLD_SIZE=8, got {world_size}. "
            "Launch with: torchrun --standalone --nproc_per_node=8 Mikey_1.1/train_gpt_8xgpu.py"
        )
    if 8 % world_size != 0:
        raise RuntimeError(f"world_size must divide 8 for Mikey grad accumulation, got {world_size}")
    ...
    log0("condition_id:mikey_1p1_8x_seed444")
    ...
    log0(f"condition:KERNEL_SMOKE_ATTENTION_MODE={KERNEL_SMOKE_ATTENTION_MODE or 'fa3_original'}")
    log0(f"condition:KERNEL_SMOKE_ONLY={os.environ.get('KERNEL_SMOKE_ONLY', '0')}")
    if KERNEL_SMOKE_ATTENTION_MODE:
        log0(f"condition:WHALE_BWD_VARIANT={os.environ.get('WHALE_BWD_VARIANT')}")
        log0(f"condition:WHALE_BWD_Q_CONFIG={os.environ.get('WHALE_BWD_Q_CONFIG')}")
        log0(f"condition:WHALE_BWD_Q_MAXNREG={os.environ.get('WHALE_BWD_Q_MAXNREG')}")
        log0(f"condition:WHALE_BWD_KV_TMA_CONFIG={os.environ.get('WHALE_BWD_KV_TMA_CONFIG')}")
        log0(f"condition:WHALE_BWD_KV_TMA_MAXNREG={os.environ.get('WHALE_BWD_KV_TMA_MAXNREG')}")
    ...
    if os.environ.get("KERNEL_SMOKE_ONLY", "0") == "1":
        log0("kernel_smoke_only:exit_before_gptq_export_eval=1")
        if distributed:
