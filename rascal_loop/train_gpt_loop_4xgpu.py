...
os.environ.setdefault("RUN_ID", "rascal_loop_4x_seed444_" + time.strftime("%Y%m%d_%H%M%S"))
...
def main() -> None:
    ...
    if world_size != 4:
        raise ValueError(
            f"Rascal loop 4x requires WORLD_SIZE=4, got {world_size}. "
            "Launch with: torchrun --standalone --nproc_per_node=4 rascal_loop/train_gpt_loop_4xgpu.py"
        )
    ...
    log0("condition_id:rascal_loop_4x_seed444")
    log0("run_label:mechanics_proxy source_record:Rascal_II_8xH100_seed444 axis:depth_recurrence")
    log0("changed_fields:gpu_count,world_size,grad_accum_steps,wallclock,depth_recurrence")
