...
class Rotary(nn.Module):
	...
	def forward_at(self,positions,dtype):
		inv_freq=self.inv_freq.to(positions.device).to(torch.float32);freqs=positions.float().unsqueeze(-1)*inv_freq;return freqs.cos().unsqueeze(1).to(dtype),freqs.sin().unsqueeze(1).to(dtype)
...
class CausalSelfAttention(nn.Module):
	...
	def _xsa_efficient_packed(self,y,v):total,H,D=y.shape;Hkv=v.size(-2);group=H//Hkv;y_g=y.reshape(total,Hkv,group,D);vn=F.normalize(v,dim=-1).unsqueeze(-2);proj=(y_g*vn).sum(dim=-1,keepdim=True)*vn;return(y_g-proj).reshape(total,H,D)
	def forward_routed(self,x_packed,positions,cu_seqlens,max_seqlen):
		total,dim=x_packed.shape;q=self.c_q(x_packed).reshape(total,self.num_heads,self.head_dim);k=self.c_k(x_packed).reshape(total,self.num_kv_heads,self.head_dim);v=self.c_v(x_packed).reshape(total,self.num_kv_heads,self.head_dim);q=F.rms_norm(q,(q.size(-1),));k=F.rms_norm(k,(k.size(-1),));cos,sin=self.rotary.forward_at(positions,q.dtype);q=apply_rotary_emb(q,cos,sin,self.rope_dims);k=apply_rotary_emb(k,cos,sin,self.rope_dims);q=q*self.q_gain.to(dtype=q.dtype)[None,:,None];y=flash_attn_3_varlen_func(q,k,v,cu_seqlens,cu_seqlens,max_seqlen,max_seqlen,causal=True)
		if self.use_xsa:y=self._xsa_efficient_packed(y,v)
		return self.proj(y.reshape(total,dim))
	def forward(self,x):
...
class Block(nn.Module):
	...
	def forward(self,x,x0):
		...
		return x_out
	def forward_routed(self,x,x0,route_mask,K,alpha,score_full):
		B,T,D=x.shape;mix=self.resid_mix.to(dtype=x.dtype);x_in=mix[0][None,None,:]*x+mix[1][None,None,:]*x0;routed_x=x_in[route_mask];positions=torch.arange(T,device=x.device,dtype=torch.long).unsqueeze(0).expand(B,T);routed_positions=positions[route_mask];cu_seqlens=torch.arange(B+1,device=x.device,dtype=torch.int32)*K;attn_out=self.attn.forward_routed(self.attn_norm(routed_x)*self.ln_scale_factor,routed_positions,cu_seqlens,K)
		if self.parallel:mlp_out=self.mlp(self.mlp_norm(routed_x)*self.ln_scale_factor);delta=self.attn_scale.to(routed_x.dtype)[None,:]*attn_out+self.mlp_scale.to(routed_x.dtype)[None,:]*mlp_out
		else:attn_delta=self.attn_scale.to(routed_x.dtype)[None,:]*attn_out;intermediate=routed_x+attn_delta;mlp_out=self.mlp(self.mlp_norm(intermediate)*self.ln_scale_factor);delta=attn_delta+self.mlp_scale.to(routed_x.dtype)[None,:]*mlp_out
		routed_score=score_full[route_mask];gate=(torch.sigmoid(routed_score)*alpha.to(routed_score.dtype)).unsqueeze(-1).to(delta.dtype);delta=delta*gate;delta_full=torch.zeros_like(x_in);delta_full[route_mask]=delta.to(x_in.dtype);return x_in+delta_full
class GPT(nn.Module):
	def __init__(self,h):
		super().__init__()
		...
		_seen=set();self.enc_revisit_slot=[];self.dec_revisit_slot=[];_slot=0
		for _idx in self.encoder_indices:
			if _idx in _seen:self.enc_revisit_slot.append(_slot);_slot+=1
			else:self.enc_revisit_slot.append(-1);_seen.add(_idx)
		for _idx in self.decoder_indices:
			if _idx in _seen:self.dec_revisit_slot.append(_slot);_slot+=1
			else:self.dec_revisit_slot.append(-1);_seen.add(_idx)
		self.num_revisits=_slot;self.loop_router_enabled=h.loop_router_enabled and self.num_revisits>0;self.loop_capacity_frac=h.loop_capacity_frac
		if self.loop_router_enabled:self.loop_routers=nn.ModuleList([CastedLinear(h.model_dim,1,bias=False)for _ in range(self.num_revisits)]);self.loop_alphas=nn.Parameter(torch.ones(self.num_revisits,dtype=torch.float32))
		else:self.loop_routers=None;self.loop_alphas=None
		self._init_weights()
	...
	def _run_block(self,x,x0,layer_idx,slot):
		if slot<0 or not self.loop_router_enabled:return self.blocks[layer_idx](x,x0)
		score=self.loop_routers[slot](x).squeeze(-1);T=x.size(1);K=max(1,int(self.loop_capacity_frac*T));topk_idx=score.topk(K,dim=-1,sorted=False).indices;topk_idx,_=topk_idx.sort(dim=-1);route_mask=torch.zeros_like(score,dtype=torch.bool);route_mask.scatter_(1,topk_idx,True);return self.blocks[layer_idx].forward_routed(x,x0,route_mask,K,self.loop_alphas[slot],score)
	def forward_logits(self,input_ids):
		x=self.tok_emb(input_ids);x=F.rms_norm(x,(x.size(-1),))
		if self.embed_proj is not None:x=self.embed_proj(x)
		x0=x;skips=[]
		if self.looping_active:enc_pairs=list(zip(self.encoder_indices,self.enc_revisit_slot));dec_pairs=list(zip(self.decoder_indices,self.dec_revisit_slot))
		else:enc_pairs=[(i,-1)for i in range(self.num_encoder_layers)];dec_pairs=[(i,-1)for i in range(self.num_encoder_layers,self.num_encoder_layers+self.num_decoder_layers)]
		for(i,slot)in enc_pairs:x=self._run_block(x,x0,i,slot);skips.append(x)
		for(skip_idx,(i,slot))in enumerate(dec_pairs):
...
class Optimizers:
	def __init__(self,h,base_model):
		...
		if base_model.loop_alphas is not None:scalar_params.append(base_model.loop_alphas)
		if base_model.loop_routers is not None:
			for r in base_model.loop_routers:scalar_params.append(r.weight)
