...
class GPT(nn.Module):
	def __init__(self,h):
		super().__init__()
		if h.logit_softcap<=_E:raise ValueError(f"logit_softcap must be positive, got {h.logit_softcap}")
...
def _compress(data,compressor):
	...
	elif compressor==_M:import brotli;return brotli.compress(data,quality=11)
	raise ValueError(f"Unknown compressor: {compressor!r}")
def _decompress(data,compressor):
	if compressor=='lzma':raw=lzma.decompress(data)
	elif compressor==_M:import brotli;raw=brotli.decompress(data)
