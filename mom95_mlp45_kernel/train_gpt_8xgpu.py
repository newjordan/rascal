...
os.environ.setdefault("RUN_ID", "rascal_mom95_mlp45_kernel_8x_seed444_" + time.strftime("%Y%m%d_%H%M%S"))
...
class Hyperparameters:
    ...
    mlp_kernel_mode = os.environ.get("MLP_KERNEL_MODE", "triton_act").strip().lower()
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        # No CastedLinear -- weights come from banks
        self.kernel_mode = os.environ.get("MLP_KERNEL_MODE", "triton_act").strip().lower()
...
def main() -> None:
    ...
    if world_size != 8:
        raise ValueError(
            f"Rascal mom95+mlp45+kernel 8x requires WORLD_SIZE=8, got {world_size}. "
            "Launch with: torchrun --standalone --nproc_per_node=8 mom95_mlp45_kernel/train_gpt_8xgpu.py"
        )
    ...
    log0("condition_id:rascal_mom95_mlp45_kernel_8x_seed444")
    log0("run_label:promotion source_record:Rascal_II_8xH100_seed444 axis:muon_momentum_0.95+mlp_mult_4.5+kernel_triton_act")
    log0(f"kernel_knob:MLP_KERNEL_MODE={args.mlp_kernel_mode!r} kernel_axis:triton_act_leaky_relu_sq baked_default:triton_act")
    log0("changed_fields:muon_momentum,mlp_mult")
    log0("expected_metric:final_sliding_window_exact comparator:1.10986874_8x_record_baseline")
