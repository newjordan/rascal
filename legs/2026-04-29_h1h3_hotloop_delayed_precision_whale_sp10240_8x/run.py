...
TEST_ID='2026-04-29_h1h3_hotloop_delayed_precision_whale_sp10240_8x'
TEST_DATE='2026-04-29'
RUN_LABEL='standard_8x'
EXPERIMENT_CLASS='new_experiment_hotloop_precision_delay'
...
SOURCE_CONDITION_ARTIFACT='/home/frosty40/sota_rascal/legs/2026-04-29_whale_l12_dim576_matrixbits5_sp10240_8x/run.py'
SOURCE_CONDITION_SHA256='7236314b20d2cdfa0c8ea8cff4ff757989e1e556f359318abef851606584bff7'
PARENT_RUN='2026-04-29_whale_l12_dim576_matrixbits5_sp10240_8x'
HYPOTHESIS='H1H3 hot-loop delayed precision: preserve the 12L dim576 SP10240 whale MLP4.0 geometry, keep global matrix_bits int5 and embeddings int8, spend int6 only on loop segment blocks 3,4,5 attention matrices, disable loop warmup explicitly, and delay loop activation to ENABLE_LOOPING_AT=0.55.'
...
Hyperparameters.run_id='h1h3_hotloop_delayed_precision_whale_sp10240_8x_seed444'
...
Hyperparameters.attn_matrix_bits=6
Hyperparameters.hot_loop_attention_layers=(3,4,5)
...
def gptq_quantize_weight(w,H,clip_sigmas=3.,clip_range=63,block_size=128):
	...
	return Q[:,invperm],s
def is_hot_loop_attention_matrix(name,h):
	return'.attn.'in name and any(name.startswith(f'blocks.{idx}.')for idx in h.hot_loop_attention_layers)
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	for(name,tensor)in state_dict.items():
		...
		elif is_hot_loop_attention_matrix(name,h):bits=h.attn_matrix_bits
...
def train_model(h,device,val_data):
	...
	if h.warmup_steps>0:
		...
		if h.num_loops>0 and h.loop_warmup_enabled:
		...
		elif h.num_loops>0:log('loop_warmup:disabled explicitly')
