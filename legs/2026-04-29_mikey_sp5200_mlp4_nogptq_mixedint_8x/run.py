...
TEST_ID='2026-04-29_mikey_sp5200_mlp4_nogptq_mixedint_8x'
TEST_DATE='2026-04-29'
RUN_LABEL='standard_8x_nogptq_mixedint'
...
HYPOTHESIS='SP5200 MLP4 PR1493 parity swing with GPTQ disabled: spend the 12s reserve on training, then use fast rowwise mixed int with embeddings int8, all attention int8, loop MLP L3-5 int7, other MLP int6, sliding eval, and legal score-first TTT.'
...
Hyperparameters.run_id='mikey_sp5200_mlp4_nogptq_mixedint_8x_seed1337'
...
Hyperparameters.gptq_reserve_seconds=0.
Hyperparameters.quant_backend='rowstd'
...
def rowstd_quantize_weight(w,clip_sigmas=3.,clip_range=63):
	W=w.float().contiguous();row_std=W.std(dim=1);s=(clip_sigmas*row_std/clip_range).clamp_min(1e-10).to(torch.float16);sf=s.float().view(-1,*([1]*(W.ndim-1)));q=torch.clamp(torch.round(W/sf),-clip_range,clip_range).to(torch.int8);return q,s
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	return result,meta
def rowstd_mixed_quantize(state_dict,h):
	result={};meta={}
	for(name,tensor)in state_dict.items():
		t=tensor.detach().cpu().contiguous()
		if not t.is_floating_point()or t.numel()<=65536:result[name]=t.to(torch.float16)if t.is_floating_point()else t;meta[name]='passthrough (float16)';continue
		cs=h.embed_clip_sigmas if'tok_emb'in name else h.matrix_clip_sigmas;bits=quant_bits_for_name(name,h);q,s=rowstd_quantize_weight(t,clip_sigmas=cs,clip_range=2**(bits-1)-1);result[name+'.q']=q;result[name+'.scale']=s;meta[name]=f"rowstd (int{bits})"
	categories=collections.defaultdict(set)
	for(name,cat)in meta.items():short=re.sub('\\.\\d+$','',re.sub('blocks\\.\\d+','blocks',name));categories[cat].add(short)
	log('Quantized weights:')
	for cat in sorted(categories):log(f"  {cat}: {", ".join(sorted(categories[cat]))}")
...
def serialize(h,base_model,code):
	...
	device=torch.device('cuda',h.local_rank);backend=getattr(h,'quant_backend','gptq')
	if backend=='rowstd':
		log('GPTQ:disabled; using fast rowstd mixed quantization');quant_result,quant_meta=rowstd_mixed_quantize(sd_cpu,h)
	elif backend=='gptq':
	...
	else:raise ValueError(f"Unknown quant_backend: {backend}")
	...
	if h.is_main_process:
		...
		if bytes_total>h.size_cap_bytes:raise RuntimeError(f"submission size {bytes_total} exceeds cap {h.size_cap_bytes}")
