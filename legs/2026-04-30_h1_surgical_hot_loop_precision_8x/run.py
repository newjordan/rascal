...
HYPOTHESIS='H1 surgical hot-loop precision: keep 12L dim576 SP10240 whale with MLP4.0 and matrix_bits int5, but spend int6 only on attention matrices in loop segment blocks 3, 4, and 5; embeddings stay int8.'
...
Hyperparameters.run_id='h1_surgical_hot_loop_precision_8x_seed444'
...
Hyperparameters.hot_loop_start=3
Hyperparameters.hot_loop_end=5
Hyperparameters.hot_loop_attn_bits=6
...
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	for(name,tensor)in state_dict.items():
		...
		if'tok_emb'in name:bits=h.embed_bits
			...
			hot_attn=re.match(r'blocks\.(\d+)\.attn\.',name)
			bits=h.hot_loop_attn_bits if hot_attn and h.hot_loop_start<=int(hot_attn.group(1))<=h.hot_loop_end else h.matrix_bits
