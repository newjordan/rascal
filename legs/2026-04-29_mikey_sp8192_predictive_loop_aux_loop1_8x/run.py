...
Hyperparameters.run_id='mikey_sp8192_predictive_loop_aux_loop1_mlp4_qk525_8x_seed1337'
...
Hyperparameters.predictive_loop_enabled=True
Hyperparameters.predictive_loop_aux_weight=.1
Hyperparameters.predictive_loop_pos_shift=1
Hyperparameters.predictive_loop_normalize='rmsnorm'
...
class Rotary(nn.Module):
	...
	def forward_shifted(self,seq_len,device,dtype,shift):
		rd=self.rope_dims
		if seq_len>self.train_seq_len:scale=seq_len/self.train_seq_len;new_base=self.base*scale**(rd/(rd-2));inv_freq=1./new_base**(torch.arange(0,rd,2,dtype=torch.float32,device=device)/rd)
		else:inv_freq=self.inv_freq.to(device)
		t=torch.arange(seq_len,device=device,dtype=inv_freq.dtype)+float(shift);freqs=torch.outer(t,inv_freq);cos=freqs.cos()[None,:,None,:].to(dtype=dtype);sin=freqs.sin()[None,:,None,:].to(dtype=dtype);return cos,sin
...
class CausalSelfAttention(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len):
		...
		kv_dim=self.num_kv_heads*self.head_dim;self.c_q=CastedLinear(dim,dim,bias=False);self.c_k=CastedLinear(dim,kv_dim,bias=False);self.c_v=CastedLinear(dim,kv_dim,bias=False);self.proj=CastedLinear(dim,dim,bias=False);self.proj._zero_init=True;self.q_gain=nn.Parameter(torch.full((num_heads,),qk_gain_init,dtype=torch.float32));self.rope_dims=0;self.rotary=Rotary(self.head_dim,base=rope_base,train_seq_len=train_seq_len);self.use_xsa=False;self.q_pos_shift=0
	...
	def forward(self,x):
		...
		if self.q_pos_shift!=0:q_cos,q_sin=self.rotary.forward_shifted(seqlen,x.device,q.dtype,self.q_pos_shift);q=apply_rotary_emb(q,q_cos,q_sin,self.rope_dims)
		else:q=apply_rotary_emb(q,cos,sin,self.rope_dims)
		k=apply_rotary_emb(k,cos,sin,self.rope_dims);q=q*self.q_gain.to(dtype=q.dtype)[None,None,:,None];y=flash_attn_3_func(q,k,v,causal=True)
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.looping_active=False;self.loop_start_idx=h.loop_start;self.loop_end_idx=h.loop_end;self.num_loops=h.num_loops
		if h.num_loops>0:
		...
		self.predictive_loop_enabled=h.predictive_loop_enabled and h.predictive_loop_aux_weight>0.;self.predictive_loop_pos_shift=h.predictive_loop_pos_shift;self.predictive_loop_normalize=h.predictive_loop_normalize;self.predictive_loop_aux_weight_runtime=float(h.predictive_loop_aux_weight);self.predictive_capture=False;self._pred_h_actual=None;self._pred_h_forecast=None;self._last_aux_mse=torch.zeros(());self._init_weights()
	...
	def _run_loop_block_shifted(self,x,x0,shift):
		for i in range(self.loop_start_idx,self.loop_end_idx+1):self.blocks[i].attn.q_pos_shift=shift
		try:
			for i in range(self.loop_start_idx,self.loop_end_idx+1):x=self.blocks[i](x,x0)
		finally:
			for i in range(self.loop_start_idx,self.loop_end_idx+1):self.blocks[i].attn.q_pos_shift=0
		return x
	def forward_logits(self,input_ids):
		...
		do_predict=self.predictive_capture and self.looping_active and self.num_loops>0;final_start=-1;seg_len=self.loop_end_idx-self.loop_start_idx+1;x_forecast=None;h_after=None
		if do_predict:enc_iter=list(enc_iter);final_start=self._find_last_loop_seg_start(enc_iter)
		for(pos,i)in enumerate(enc_iter):
			if do_predict and final_start>=0 and pos==final_start:x_forecast=self._run_loop_block_shifted(x,x0,self.predictive_loop_pos_shift)
			x=self.blocks[i](x,x0);skips.append(x)
			if do_predict and final_start>=0 and pos==final_start+seg_len-1:h_after=x
		if do_predict and h_after is not None and x_forecast is not None:self._pred_h_actual=h_after;self._pred_h_forecast=x_forecast
	...
	def _find_last_loop_seg_start(self,idx_list):
		seg=list(range(self.loop_start_idx,self.loop_end_idx+1));L=len(seg)
		if L==0 or len(idx_list)<L:return -1
		for s in range(len(idx_list)-L,-1,-1):
			if idx_list[s:s+L]==seg:return s
		return -1
	def forward(self,input_ids,target_ids):
		want_aux=self.predictive_loop_enabled and self.training and self.looping_active and self.num_loops>0
		if want_aux:self._pred_h_actual=None;self._pred_h_forecast=None;self.predictive_capture=True
		try:logits=self.forward_logits(input_ids);ce=F.cross_entropy(logits.reshape(-1,logits.size(-1)).float(),target_ids.reshape(-1),reduction='mean')
		finally:
			if want_aux:self.predictive_capture=False
		if not want_aux or self._pred_h_forecast is None or self._pred_h_actual is None:self._last_aux_mse=torch.zeros((),device=input_ids.device,dtype=ce.dtype);return ce
		shift=max(int(self.predictive_loop_pos_shift),1);h_act=self._pred_h_actual;h_pred=self._pred_h_forecast
		if self.predictive_loop_normalize=='rmsnorm':h_act_n=F.rms_norm(h_act.float(),(h_act.size(-1),));h_pred_n=F.rms_norm(h_pred.float(),(h_pred.size(-1),))
		else:h_act_n=h_act.float();h_pred_n=h_pred.float()
		pred_slice=h_pred_n[:,:-shift,:];target_slice=h_act_n[:,shift:,:].detach();mse=F.mse_loss(pred_slice,target_slice,reduction='mean').to(dtype=ce.dtype);self._pred_h_actual=None;self._pred_h_forecast=None;self._last_aux_mse=mse.detach();return ce+self.predictive_loop_aux_weight_runtime*mse
...
def deserialize(h,device):
	...
	quant_state=torch.load(io.BytesIO(_decompress(quant_blob_disk,h.compressor)),map_location='cpu');deq_state=dequantize_mixed(quant_state['w'],quant_state['m'],sd_cpu);eval_model.load_state_dict(deq_state,strict=True);eval_model.predictive_loop_enabled=False;eval_model.predictive_capture=False;return eval_model
...
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	for ci in range(num_chunks):
		...
		if not is_last_chunk and h.ttt_epochs>0:
			base_model.train();base_model.predictive_loop_enabled=False;base_model.predictive_capture=False;chunk_seqs=(chunk_end-chunk_start)//seq_len
			if chunk_seqs>0:
...
def train_model(h,device,val_data):
	base_model=GPT(h).to(device).bfloat16();restore_fp32_params(base_model);use_aux=h.predictive_loop_enabled and h.predictive_loop_aux_weight>0.;compiled_model=torch.compile(base_model,dynamic=False,fullgraph=True);train_inner=base_model if use_aux else compiled_model
	if h.distributed:model=DDP(train_inner,device_ids=[h.local_rank],broadcast_buffers=False)
	else:model=train_inner
	log(f"model_params:{sum(p.numel()for p in base_model.parameters())}");log(f"predictive_loop:enabled={use_aux} aux_weight={h.predictive_loop_aux_weight} pos_shift={h.predictive_loop_pos_shift} normalize={h.predictive_loop_normalize}");optimizers=Optimizers(h,base_model);train_loader=ShuffledSequenceLoader(h,device);max_wallclock_ms=1e3*h.max_wallclock_seconds if h.max_wallclock_seconds>0 else None
	...
	def step_fn(step,lr_scale):
		optimizers.zero_grad_all();train_loss=torch.zeros((),device=device);train_aux=torch.zeros((),device=device)
		for micro_step in range(h.grad_accum_steps):
			...
			train_loss+=loss.detach();train_aux+=base_model._last_aux_mse.detach().to(device=device,dtype=loss.dtype);(loss/h.grad_accum_steps).backward()
		train_loss/=h.grad_accum_steps;train_aux/=h.grad_accum_steps;frac=min(step/h.muon_momentum_warmup_steps,1.)if h.muon_momentum_warmup_steps>0 else 1.;muon_momentum=(1-frac)*h.muon_momentum_warmup_start+frac*h.muon_momentum
		...
		optimizers.step();return train_loss,train_aux
	if h.warmup_steps>0:
	...
	while True:
		...
		train_loss,train_aux=step_fn(step,scale)
		with torch.no_grad():
		...
		if should_log_train:tok_per_sec=step*h.train_batch_tokens/(approx_training_time_ms/1e3);ce_only=(train_loss-h.predictive_loop_aux_weight*train_aux).item()if use_aux else train_loss.item();log(f"{step}/{h.iterations} train_loss: {train_loss.item():.4f} train_ce: {ce_only:.4f} train_aux_mse: {train_aux.item():.6f} train_time: {approx_training_time_ms/60000:.1f}m tok/s: {tok_per_sec:.0f}")
	...
	log(f"peak memory allocated: {torch.cuda.max_memory_allocated()//1024//1024} MiB reserved: {torch.cuda.max_memory_reserved()//1024//1024} MiB");log('ema:applying EMA weights');current_state=base_model.state_dict();avg_state={name:t.to(dtype=current_state[name].dtype)for(name,t)in ema_state.items()};base_model.load_state_dict(avg_state,strict=True);base_model.predictive_loop_enabled=False;base_model.predictive_capture=False;return base_model,compiled_model
