...
os.environ.setdefault("RUN_ID", "rascal_sp4096_mlp35_2x_seed444_" + time.strftime("%Y%m%d_%H%M%S"))

TEST_ID = "2026-04-29_rascal_sp4096_mlp35_2x"
RUN_LABEL = "new_experiment_mlp35_2x"
...
def main() -> None:
    ...
    if world_size != 2:
        raise ValueError(
            f"{TEST_ID} requires WORLD_SIZE=2, got {world_size}. "
            "Launch with: torchrun --standalone --nproc_per_node=2 run.py"
        )
    ...
    log0("changed_fields:MLP_MULT 3.0->3.5, gpus 8->2, world_size 8->2, grad_accum_steps 1->4, wallclock 600->2400")
    log0(f"condition:BUILD_SECONDS={BUILD_SECONDS} EVAL_SECONDS={EVAL_SECONDS} SIZE_CAP_BYTES={SIZE_CAP_BYTES}")
    log0("expected_metric:final_sliding_window_exact comparator:quarantined_4k_signal_only")
