...
command -v lrzip >/dev/null || { echo "missing lrzip" >&2; exit 2; }
...
if [ "${train_count}" -lt 80 ] || [ "${val_count}" -lt 1 ] || [ "${val_bytes_count}" -ne "${val_count}" ]; then
  echo "incomplete PR1855 CaseOps data: train=${train_count} val=${val_count} val_bytes=${val_bytes_count}" >&2
  exit 2
fi
...
export SEED=42
export ENABLE_LOOPING_AT=0.50
export RUN_ID="pr1855_sp8192_lqer_smeargate_loop_late050_8x_seed42"
