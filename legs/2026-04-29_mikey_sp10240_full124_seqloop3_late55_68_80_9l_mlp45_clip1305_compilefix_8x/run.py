...
TEST_ID='2026-04-29_mikey_sp10240_full124_seqloop3_late55_68_80_9l_mlp45_clip1305_compilefix_8x'
...
HYPOTHESIS='SP10240 full124 9L MLP4.5 sequential loop3 swing with compile repair: test whether a wider fast 9L body can absorb staged extra loop pressure at frac 0.55/0.68/0.80 while disabling fullgraph compile so dynamic loop paths can recompile instead of failing.'
...
Hyperparameters.run_id='mikey_sp10240_full124_seqloop3_late55_68_80_9l_mlp45_clip1305_compilefix_8x_seed444'
...
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	log(f"ttt:start chunks={num_chunks} ttt_lr={h.ttt_lr} ttt_epochs={h.ttt_epochs}");compiled_logits=torch.compile(base_model.forward_logits,dynamic=False,fullgraph=getattr(h,'compile_fullgraph',True));loss_sum=torch.zeros((),device=device,dtype=torch.float64);token_count=torch.zeros((),device=device,dtype=torch.float64);byte_count=torch.zeros((),device=device,dtype=torch.float64);ttt_params=[p for p in base_model.parameters()]
...
def train_model(h,device,val_data):
	base_model=GPT(h).to(device).bfloat16();restore_fp32_params(base_model);compiled_model=torch.compile(base_model,dynamic=False,fullgraph=getattr(h,'compile_fullgraph',True))
...
def train_and_eval(h,device):
	...
	compiled_model=torch.compile(eval_model,dynamic=False,fullgraph=getattr(h,'compile_fullgraph',True));timed_eval('quantized',eval_val,h,device,val_data,compiled_model)
