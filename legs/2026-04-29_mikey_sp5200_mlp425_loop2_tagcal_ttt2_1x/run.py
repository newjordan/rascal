...
TEST_ID='2026-04-29_mikey_sp5200_mlp425_loop2_tagcal_ttt2_1x'
TEST_DATE='2026-04-29'
RUN_LABEL='mechanics_1x'
...
PARENT_RUN='2026-04-29_mikey_sp5200_opt_mlp425_loop2_8x'
HYPOTHESIS='1x mechanics proxy for closest-known SP5200 MLP4.25 loop2 runner with legal score-first chunk-vector tag calibration stacked before TTT; TTT epochs reduced to 2 to buy eval time for the calibrator.'
...
Hyperparameters.data_dir='/workspace/data/'
Hyperparameters.seed=1337
Hyperparameters.run_id='mikey_sp5200_mlp425_loop2_tagcal_ttt2_1x_seed1337'
Hyperparameters.max_wallclock_seconds=float(BUILD_SECONDS)
Hyperparameters.vocab_size=5200
Hyperparameters.num_layers=11
Hyperparameters.xsa_last_n=11
Hyperparameters.mlp_mult=4.25
...
Hyperparameters.enable_looping_at=.35
...
Hyperparameters.datasets_dir=os.path.join(Hyperparameters.data_dir,'datasets',f"fineweb10B_sp{Hyperparameters.vocab_size}")
...
def _tagcal_tags(h,val_data,scored_logits,prev_ids):
	top_ids=scored_logits.detach().argmax(dim=-1)
	top_space=val_data.has_leading_space_lut[top_ids].to(torch.long);top_boundary=val_data.is_boundary_token_lut[top_ids].to(torch.long)
	prev_space=val_data.has_leading_space_lut[prev_ids].to(torch.long);prev_boundary=val_data.is_boundary_token_lut[prev_ids].to(torch.long)
	return(top_space+2*top_boundary+4*prev_space+8*prev_boundary).remainder(h.tagcal_tags)
def _tagcal_make_bias(h,counts,totals):
	if totals.sum().item()<h.tagcal_min_count:return None
	logp=torch.log(counts+h.tagcal_smoothing)-torch.log(totals[:,None]+h.tagcal_smoothing*h.vocab_size)
	bias=(h.tagcal_alpha*(logp+math.log(h.vocab_size))).clamp(-h.tagcal_clip,h.tagcal_clip)
	inactive=totals<h.tagcal_min_count
	if inactive.any():bias[inactive]=0.
	return bias
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	tagcal_on=bool(getattr(h,'tagcal_enabled',False));log(f"ttt:start chunks={num_chunks} ttt_lr={h.ttt_lr} ttt_epochs={h.ttt_epochs} tagcal={int(tagcal_on)}");compiled_logits=torch.compile(base_model.forward_logits,dynamic=False,fullgraph=True);loss_sum=torch.zeros((),device=device,dtype=torch.float64);token_count=torch.zeros((),device=device,dtype=torch.float64);byte_count=torch.zeros((),device=device,dtype=torch.float64);ttt_params=[p for p in base_model.parameters()]
	tagcal_counts=torch.zeros((h.tagcal_tags,h.vocab_size),device=device,dtype=torch.float32)if tagcal_on else None;tagcal_totals=torch.zeros((h.tagcal_tags,),device=device,dtype=torch.float32)if tagcal_on else None;tagcal_bias=None
	...
	for ci in range(num_chunks):
		...
		chunk_start=ci*ttt_chunk;chunk_end=min((ci+1)*ttt_chunk,total_tokens);my_s=len(windows)*rank//world_size;my_e=len(windows)*(rank+1)//world_size;my_windows=windows[my_s:my_e];base_model.eval();chunk_tag_counts=torch.zeros_like(tagcal_counts)if tagcal_on else None
		with torch.no_grad():
			for bi in range(0,len(my_windows),batch_seqs):
				batch_ws=my_windows[bi:bi+batch_seqs];bsz=len(batch_ws);x_batch=torch.zeros(bsz,seq_len,dtype=torch.int64,device=device);y_batch=torch.zeros(bsz,seq_len,dtype=torch.int64,device=device);wlens=[]
				...
				nll=None if tagcal_on else F.cross_entropy(logits.reshape(-1,logits.size(-1)).float(),y_batch.reshape(-1),reduction='none').reshape(bsz,seq_len)
				batch_tags=[];batch_tgts=[]
				for(i,ws)in enumerate(batch_ws):
					wlen=wlens[i];s=0 if ws==0 else context_size;tgt=y_batch[i,s:wlen];prev=x_batch[i,s:wlen]
					if tagcal_on:
						scored_logits=logits[i,s:wlen].float();tags=_tagcal_tags(h,val_data,scored_logits,prev);adj_logits=scored_logits if tagcal_bias is None else scored_logits+tagcal_bias.index_select(0,tags);scored_nll=F.cross_entropy(adj_logits,tgt,reduction='none').to(torch.float64);batch_tags.append(tags);batch_tgts.append(tgt)
					else:scored_nll=nll[i,s:wlen].to(torch.float64)
				...
				if tagcal_on and batch_tags:
					tags=torch.cat(batch_tags).to(torch.long);tgts=torch.cat(batch_tgts).to(torch.long);flat=tags*h.vocab_size+tgts;chunk_tag_counts+=torch.bincount(flat,minlength=h.tagcal_tags*h.vocab_size).reshape(h.tagcal_tags,h.vocab_size).to(torch.float32)
		if tagcal_on:
			if dist.is_available()and dist.is_initialized():dist.all_reduce(chunk_tag_counts,op=dist.ReduceOp.SUM)
			tagcal_counts.mul_(h.tagcal_decay).add_(chunk_tag_counts);tagcal_totals.copy_(tagcal_counts.sum(dim=1));tagcal_bias=_tagcal_make_bias(h,tagcal_counts,tagcal_totals)
			if h.is_main_process and(ci<3 or(ci+1)%128==0):active=int((tagcal_totals>=h.tagcal_min_count).sum().item());log(f"tagcal:update chunk:{ci+1}/{num_chunks} active_tags:{active}/{h.tagcal_tags} tokens:{tagcal_totals.sum().item():.0f} alpha:{h.tagcal_alpha}")
