...
TEST_ID='2026-04-30_mikey_sp10240_10l_mlp4_seqloop3_step1500_3000_4500_maxstep_8x'
...
PARENT_RUN='2026-04-30_mikey_sp10240_10l_mlp4_seqloop3_early45_60_72_maxstep_8x'
HYPOTHESIS='Step-scheduled 10L seqloop3 escalation: turn the first loop stage on at exactly step 1500, then continue staged loop pressure at steps 3000 and 4500 so the model gets a longer loop-settling window under the 600s budget.'
...
Hyperparameters.run_id='mikey_sp10240_10l_mlp4_seqloop3_step1500_3000_4500_maxstep_8x_seed444'
...
Hyperparameters.num_loops=3
Hyperparameters.loop_start=3
Hyperparameters.loop_end=5
Hyperparameters.enable_looping_at=.45
Hyperparameters.loop_schedule_enabled=True
Hyperparameters.loop_schedule_fracs=(.45,.60,.72)
Hyperparameters.loop_schedule_steps=(1500,3000,4500)
Hyperparameters.compile_fullgraph=False
Hyperparameters.compile_dynamic=True
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.looping_active=False;self.active_loop_extra_loops=0;self.max_loop_extra_loops=h.num_loops;self.loop_paths={}
		if h.num_loops>0:
			loop_seg=list(range(h.loop_start,h.loop_end+1));all_indices=list(range(h.loop_start))
			for extra_loops in range(1,h.num_loops+1):
				all_indices=list(range(h.loop_start))
				for _ in range(extra_loops+1):all_indices.extend(loop_seg)
				all_indices.extend(range(h.loop_end+1,h.num_layers));num_enc=len(all_indices)//2;self.loop_paths[extra_loops]=(all_indices[:num_enc],all_indices[num_enc:])
			self.set_loop_extra_loops(h.num_loops);self.active_loop_extra_loops=0;self.looping_active=False
	...
	def set_loop_extra_loops(self,extra_loops):
		extra_loops=max(0,min(int(extra_loops),self.max_loop_extra_loops));self.active_loop_extra_loops=extra_loops
		if extra_loops<=0:self.looping_active=False;return
		self.looping_active=True;self.encoder_indices,self.decoder_indices=self.loop_paths[extra_loops]
...
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	log(f"ttt:start chunks={num_chunks} ttt_lr={h.ttt_lr} ttt_epochs={h.ttt_epochs}");compiled_logits=torch.compile(base_model.forward_logits,dynamic=getattr(h,'compile_dynamic',False),fullgraph=getattr(h,'compile_fullgraph',True));loss_sum=torch.zeros((),device=device,dtype=torch.float64);token_count=torch.zeros((),device=device,dtype=torch.float64);byte_count=torch.zeros((),device=device,dtype=torch.float64);ttt_params=[p for p in base_model.parameters()]
...
def train_model(h,device,val_data):
	base_model=GPT(h).to(device).bfloat16();restore_fp32_params(base_model);compiled_model=torch.compile(base_model,dynamic=getattr(h,'compile_dynamic',False),fullgraph=getattr(h,'compile_fullgraph',True))
	...
	if h.warmup_steps>0:
		...
		if h.num_loops>0:
			if getattr(h,'loop_schedule_enabled',False):
				stage_warmup_steps=max(1,h.warmup_steps//4)
				for extra_loops in range(1,h.num_loops+1):
					base_model.set_loop_extra_loops(extra_loops);log(f"loop_warmup:stage extra_loops:{extra_loops} encoder:{base_model.encoder_indices} decoder:{base_model.decoder_indices}")
					for warmup_step in range(stage_warmup_steps):
						step_fn(warmup_step,1.)
						if warmup_step+1==stage_warmup_steps:log(f"loop_warmup_stage:{extra_loops} steps:{stage_warmup_steps}")
				base_model.set_loop_extra_loops(0)
			else:
	...
	while True:
		...
		if h.num_loops>0:
			if getattr(h,'loop_schedule_enabled',False):
				target_extra=0
				if getattr(h,'loop_schedule_steps',None):
					for idx,stage_step in enumerate(h.loop_schedule_steps,start=1):
						if step>=stage_step:target_extra=idx
				else:
					for idx,stage_frac in enumerate(h.loop_schedule_fracs,start=1):
						if frac>=stage_frac:target_extra=idx
				target_extra=min(target_extra,h.num_loops)
				if target_extra!=base_model.active_loop_extra_loops:base_model.set_loop_extra_loops(target_extra);log(f"layer_loop:stage extra_loops:{target_extra} step:{step} frac:{frac:.3f} encoder:{base_model.encoder_indices} decoder:{base_model.decoder_indices}")
			elif not base_model.looping_active and frac>=h.enable_looping_at:base_model.looping_active=True;log(f"layer_loop:enabled step:{step} frac:{frac:.3f} encoder:{base_model.encoder_indices} decoder:{base_model.decoder_indices}")
...
def train_and_eval(h,device):
	...
	compiled_model=torch.compile(eval_model,dynamic=getattr(h,'compile_dynamic',False),fullgraph=getattr(h,'compile_fullgraph',True));timed_eval('quantized',eval_val,h,device,val_data,compiled_model)
