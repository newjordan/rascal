...
TEST_ID='2026-04-29_mikey_sp10240_mlp375_evalonly_ttt5_1x'
TEST_DATE='2026-04-29'
RUN_LABEL='mechanics_1x_eval_only'
...
PARENT_RUN='pod_pulls/8x_35002131_20260429_sp10240_mlp375_promising_20260429_202105/final_model.int6.ptz'
HYPOTHESIS='Eval-only 1x proxy for the promising SP10240 11L MLP3.75 quantized artifact. Replays quantized, sliding, and TTT5 without retraining to test whether another TTT pass still improves the final evaluation path after TTT4 reached 1.08057217.'
...
Hyperparameters.run_id='mikey_sp10240_mlp375_evalonly_ttt5_1x_seed444'
...
def eval_only(h,device):
	random.seed(h.seed);np.random.seed(h.seed);torch.manual_seed(h.seed);torch.cuda.manual_seed_all(h.seed);val_data=ValidationData(h,device);log(f"eval_only_artifact:{h.quantized_model_path}");log(f"val_tokens: {val_data.val_tokens.numel()-1}");eval_model=deserialize(h,device)
	if h.num_loops>0:eval_model.looping_active=True
	compiled_model=torch.compile(eval_model,dynamic=False,fullgraph=True);timed_eval('evalonly_quantized',eval_val,h,device,val_data,compiled_model)
	if h.sliding_window_enabled:timed_eval('evalonly_quantized_sliding_window',eval_val_sliding,h,device,val_data,eval_model)
	if h.ttt_enabled and h.sliding_window_enabled:
		del eval_model,compiled_model;torch._dynamo.reset();torch.cuda.empty_cache();ttt_model=deserialize(h,device)
		if h.num_loops>0:ttt_model.looping_active=True
		timed_eval(f"evalonly_quantized_ttt_epochs{h.ttt_epochs}",eval_val_ttt,h,device,val_data,ttt_model);del ttt_model
def main():
