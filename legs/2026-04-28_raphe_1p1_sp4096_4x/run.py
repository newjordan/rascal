...
os.environ.setdefault("RUN_ID", "raphe_1p1_sp4096_4x_seed444_" + time.strftime("%Y%m%d_%H%M%S"))

TEST_ID = "2026-04-28_raphe_1p1_sp4096_4x"
RUN_LABEL = "mechanics_4x"
...
def main() -> None:
    ...
    if world_size != 4:
        raise ValueError(
            f"{TEST_ID} requires WORLD_SIZE=4, got {world_size}. "
            "Launch with: torchrun --standalone --nproc_per_node=4 run.py"
        )
    ...
    log0("changed_fields:gpus 8->4, world_size 8->4, grad_accum_steps 1->2, wallclock 600->1200")
