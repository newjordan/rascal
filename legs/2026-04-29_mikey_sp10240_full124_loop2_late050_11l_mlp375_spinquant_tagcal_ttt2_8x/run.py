import collections,copy,glob,hashlib,io,lzma,math,os,queue,threading
...
TEST_ID='2026-04-29_mikey_sp10240_full124_loop2_late050_11l_mlp375_spinquant_tagcal_ttt2_8x'
...
PARENT_RUN='2026-04-29_mikey_sp10240_full124_loop2_late050_11l_mlp375_spinquant_8x plus legal score-first tagcal from 2026-04-29_mikey_sp5200_mlp425_loop2_tagcal_ttt2_8x'
HYPOTHESIS='SP10240 full124 11L MLP3.75 with loop2 late050, partial SpinQuant before GPTQ, and legal score-first chunk-vector tag calibration stacked with TTT2. This targets the final score path directly rather than another small neural capacity bump.'
...
Hyperparameters.run_id='mikey_sp10240_full124_loop2_late050_11l_mlp375_spinquant_tagcal_ttt2_8x_seed444'
...
Hyperparameters.spinquant_enabled=True
Hyperparameters.spinquant_start_layer=5
Hyperparameters.spinquant_kind='hadamard'
Hyperparameters.spinquant_seed=7919
Hyperparameters.spinquant_apply_to='both'
Hyperparameters.ttt_enabled=True
Hyperparameters.ttt_lr=.005
Hyperparameters.ttt_epochs=2
Hyperparameters.ttt_momentum=.9
Hyperparameters.tagcal_enabled=True
Hyperparameters.tagcal_tags=16
Hyperparameters.tagcal_alpha=.08
Hyperparameters.tagcal_smoothing=32.
Hyperparameters.tagcal_min_count=256.
Hyperparameters.tagcal_clip=.5
Hyperparameters.tagcal_decay=1.
...
_SPINQUANT_CACHE={}
_SQ_KEY_TO_TAG=(('.attn.c_q.weight','qkv_input'),('.attn.c_k.weight','qkv_input'),('.attn.c_v.weight','qkv_input'),('.mlp.fc.weight','mlp_input'))
def _spinquant_stable_seed(seed,layer_idx,tag):d=hashlib.sha256(f"{int(seed)}:{int(layer_idx)}:{tag}".encode('utf-8')).digest();return int.from_bytes(d[:4],'big')
def _spinquant_rotation(n,seed,layer_idx,tag,kind='hadamard'):
	key=(int(seed),int(layer_idx),tag,int(n),str(kind))
	if key in _SPINQUANT_CACHE:return _SPINQUANT_CACHE[key]
	g=torch.Generator().manual_seed(_spinquant_stable_seed(seed,layer_idx,tag))
	if kind=='hadamard':
		p=1
		while p<n:p*=2
		H=torch.ones(1,1)
		while H.shape[0]<p:H=torch.cat([torch.cat([H,H],dim=1),torch.cat([H,-H],dim=1)],dim=0)
		H=H/math.sqrt(p);D=torch.diag(torch.randint(0,2,(p,),generator=g).float()*2-1);M=(D@H)[:n,:n]
	else:M=torch.randn(n,n,generator=g)
	Q,_=torch.linalg.qr(M.double());Q=Q.float();_SPINQUANT_CACHE[key]=Q;return Q
def _spinquant_applies_to(tag,apply_to):
	if apply_to=='both':return True
	return tag==apply_to
def _spinquant_rotate_sd_and_H(sd_cpu,hessians,h):
	if not getattr(h,'spinquant_enabled',False):return 0,0
	baked_w=0;baked_h=0;missing=0
	for name in list(sd_cpu.keys()):
		tag=None
		for(suffix,t)in _SQ_KEY_TO_TAG:
			if name.endswith(suffix)and name.startswith('blocks.'):tag=t;break
		if tag is None:continue
		if not _spinquant_applies_to(tag,h.spinquant_apply_to):continue
		try:layer_idx=int(name.split('.')[1])
		except(IndexError,ValueError):continue
		if layer_idx<h.spinquant_start_layer:continue
		W=sd_cpu[name]
		if W.ndim!=2:continue
		in_dim=W.shape[1];R=_spinquant_rotation(in_dim,h.spinquant_seed,layer_idx,tag,kind=h.spinquant_kind).float().cpu()
		assert R.shape==(in_dim,in_dim),f"spinquant: R shape {tuple(R.shape)} != ({in_dim},{in_dim}) for {name}"
		orig_dtype=W.dtype;sd_cpu[name]=(W.float()@R).to(orig_dtype).contiguous();baked_w+=1
		if name in hessians:
			Hm=hessians[name];assert Hm.shape==(in_dim,in_dim),f"spinquant: H shape {tuple(Hm.shape)} != ({in_dim},{in_dim}) for {name}"
			H_dev=Hm.device;H32=Hm.float().cpu();hessians[name]=(R.T@H32@R).to(Hm.dtype).to(H_dev);baked_h+=1
		else:missing+=1
	log(f"spinquant:baked seed:{h.spinquant_seed} kind:{h.spinquant_kind} apply_to:{h.spinquant_apply_to} start_layer:{h.spinquant_start_layer} weights:{baked_w} hessians:{baked_h} missing_hessian:{missing}")
	if missing:raise RuntimeError(f"spinquant: {missing} rotated weights had no matching Hessian - would break the quantized model")
	return baked_w,baked_h
def install_spinquant_eval_hooks(eval_model,h):
	if not getattr(h,'spinquant_enabled',False):return 0
	installed=0
	for(layer_idx,block)in enumerate(eval_model.blocks):
		if layer_idx<h.spinquant_start_layer:continue
		if _spinquant_applies_to('qkv_input',h.spinquant_apply_to):
			R_qkv=_spinquant_rotation(h.model_dim,h.spinquant_seed,layer_idx,'qkv_input',kind=h.spinquant_kind)
			for lin in(block.attn.c_q,block.attn.c_k,block.attn.c_v):
				R_buf=R_qkv.to(device=lin.weight.device,dtype=lin.weight.dtype)
				def make_hook(R):
					def pre_hook(module,args):x=args[0];return(x@R.to(dtype=x.dtype),)
					return pre_hook
				lin.register_forward_pre_hook(make_hook(R_buf));installed+=1
		if _spinquant_applies_to('mlp_input',h.spinquant_apply_to):
			R_mlp=_spinquant_rotation(h.model_dim,h.spinquant_seed,layer_idx,'mlp_input',kind=h.spinquant_kind)
			lin=block.mlp.fc;R_buf=R_mlp.to(device=lin.weight.device,dtype=lin.weight.dtype)
			def make_hook(R):
				def pre_hook(module,args):x=args[0];return(x@R.to(dtype=x.dtype),)
				return pre_hook
			lin.register_forward_pre_hook(make_hook(R_buf));installed+=1
	log(f"spinquant:installed_eval_hooks:{installed} start_layer:{h.spinquant_start_layer} apply_to:{h.spinquant_apply_to}")
	return installed
def serialize(h,base_model,code):
	...
	device=torch.device('cuda',h.local_rank);log('GPTQ:collecting Hessians from calibration data...');t0=time.perf_counter();calib_loader=ShuffledSequenceLoader(h,device);hessians=collect_hessians(base_model,calib_loader,h,device,n_calibration_batches=h.gptq_calibration_batches);log(f"GPTQ:collected {len(hessians)} Hessians in {time.perf_counter()-t0:.1f}s");_spinquant_rotate_sd_and_H(sd_cpu,hessians,h);quant_result,quant_meta=gptq_mixed_quantize(sd_cpu,hessians,h);quant_buf=io.BytesIO();torch.save({'w':quant_result,'m':quant_meta},quant_buf);quant_raw=quant_buf.getvalue();quant_blob=_compress(quant_raw,h.compressor);quant_file_bytes=len(quant_blob);bytes_total=quant_file_bytes+code_bytes
	if h.is_main_process:
...
def deserialize(h,device):
	...
	quant_state=torch.load(io.BytesIO(_decompress(quant_blob_disk,h.compressor)),map_location='cpu');deq_state=dequantize_mixed(quant_state['w'],quant_state['m'],sd_cpu);eval_model.load_state_dict(deq_state,strict=True);install_spinquant_eval_hooks(eval_model,h);return eval_model
