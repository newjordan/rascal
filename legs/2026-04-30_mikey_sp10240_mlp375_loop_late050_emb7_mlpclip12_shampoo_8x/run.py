...
TEST_ID='2026-04-30_mikey_sp10240_mlp375_loop_late050_emb7_mlpclip12_shampoo_8x'
...
PARENT_RUN='2026-04-30_mikey_sp10240_mlp375_loop_late050_emb7_mlpclip12_8x'
HYPOTHESIS='Real Shampoo optimizer-axis test stacked on the clean best-body quant-policy runner: keep Mikey SP10240 11L MLP3.75 late050 emb7 MLPClip12 fixed, and replace Muon zero-power updates with a running beta2 Shampoo second-moment inverse-root preconditioner.'
...
Hyperparameters.run_id='mikey_sp10240_mlp375_loop_late050_emb7_mlpclip12_shampoo_8x_seed444'
...
Hyperparameters.mlp_clip_sigmas=12.0
Hyperparameters.shampoo_enabled=True
Hyperparameters.shampoo_beta2=.95
Hyperparameters.shampoo_eps=1e-4
Hyperparameters.shampoo_update_every=1
...
def _sym_inv_sqrt(S,eps):
	S32=S.float();S32=0.5*(S32+S32.T);eye=torch.eye(S32.size(0),device=S32.device,dtype=S32.dtype);evals,evecs=torch.linalg.eigh(S32+eps*eye);evals=evals.clamp_min(eps);return(evecs*evals.rsqrt().unsqueeze(0))@evecs.T
def shampoo_precondition(G,state,beta2,eps,update_every,step):
	U=G.float();rows,cols=U.shape;use_right=rows>=cols;dim=cols if use_right else rows;key='shampoo_right'if use_right else'shampoo_left'
	if key not in state:state[key]=torch.eye(dim,device=U.device,dtype=torch.float32);state['shampoo_inv_step']=-1
	S=state[key]
	if use_right:gram=U.T@U
	else:gram=U@U.T
	S.mul_(beta2).add_(gram,alpha=1.-beta2)
	inv_key=key+'_inv';need_update=inv_key not in state or update_every<=1 or step%update_every==0 or state.get('shampoo_inv_step',-1)<0
	if need_update:
		state[inv_key]=_sym_inv_sqrt(S,eps);state['shampoo_inv_step']=step
	P=state[inv_key]
	return(U@P if use_right else P@U).to(dtype=G.dtype)
class Muon(torch.optim.Optimizer):
	def __init__(self,params,lr,momentum,backend_steps,nesterov=True,weight_decay=.0,row_normalize=False,shampoo_enabled=False,shampoo_beta2=.0,shampoo_eps=1e-4,shampoo_update_every=1):super().__init__(params,dict(lr=lr,momentum=momentum,backend_steps=backend_steps,nesterov=nesterov,weight_decay=weight_decay,row_normalize=row_normalize,shampoo_enabled=shampoo_enabled,shampoo_beta2=shampoo_beta2,shampoo_eps=shampoo_eps,shampoo_update_every=shampoo_update_every,shampoo_step=0))
	@torch.no_grad()
	def step(self,closure=None):
		...
		for group in self.param_groups:
			...
			lr=group['lr'];momentum=group['momentum'];backend_steps=group['backend_steps'];nesterov=group['nesterov'];shampoo_enabled=group.get('shampoo_enabled',False);shampoo_beta2=group.get('shampoo_beta2',.0);shampoo_eps=group.get('shampoo_eps',1e-4);shampoo_update_every=group.get('shampoo_update_every',1);shampoo_step=group.get('shampoo_step',0);total_params=sum(int(p.numel())for p in params);updates_flat=torch.zeros(total_params,device=params[0].device,dtype=torch.bfloat16);curr=0
			for(i,p)in enumerate(params):
				if i%world_size==rank and p.grad is not None:
					...
					if shampoo_enabled:g=shampoo_precondition(g,state,shampoo_beta2,shampoo_eps,shampoo_update_every,shampoo_step)
				...
				curr+=p.numel()
			group['shampoo_step']=shampoo_step+1
...
class Optimizers:
	def __init__(self,h,base_model):
		...
		token_lr=h.tied_embed_lr if h.tie_embeddings else h.embed_lr;tok_params=[{'params':[base_model.tok_emb.weight],'lr':token_lr,'base_lr':token_lr}];self.optimizer_tok=torch.optim.AdamW(tok_params,betas=(h.beta1,h.beta2),eps=h.adam_eps,weight_decay=h.embed_wd,fused=True);self.optimizer_muon=Muon(matrix_params,lr=h.matrix_lr,momentum=h.muon_momentum,backend_steps=h.muon_backend_steps,weight_decay=h.muon_wd,row_normalize=h.muon_row_normalize,shampoo_enabled=getattr(h,'shampoo_enabled',False),shampoo_beta2=getattr(h,'shampoo_beta2',.0),shampoo_eps=getattr(h,'shampoo_eps',1e-4),shampoo_update_every=getattr(h,'shampoo_update_every',1))
...
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	for(name,tensor)in state_dict.items():
		...
		elif'.mlp.'in name:cs=getattr(h,'mlp_clip_sigmas',h.matrix_clip_sigmas);bits=h.matrix_bits
		else:cs=h.matrix_clip_sigmas;bits=h.matrix_bits
		q,s=gptq_quantize_weight(t,hessians[name],clip_sigmas=cs,clip_range=2**(bits-1)-1);result[name+'.q']=q;result[name+'.scale']=s;meta[name]=f"gptq (int{bits}, clip={cs:g})"
