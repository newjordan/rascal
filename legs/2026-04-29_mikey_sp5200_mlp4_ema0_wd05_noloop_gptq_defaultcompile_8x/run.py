...
Hyperparameters.run_id='mikey_sp5200_mlp4_ema0_wd05_noloop_gptq_defaultcompile_8x_seed1337'
...
Hyperparameters.ema_decay=0.
Hyperparameters.ema_foreach=True
Hyperparameters.data_prefetch=True
Hyperparameters.async_save=True
Hyperparameters.gpu_warmup_snapshot=True
Hyperparameters.compile_mode='default'
Hyperparameters.pack_qkv=False
Hyperparameters.rotary_precompute=True
Hyperparameters.ttt_enabled=True
Hyperparameters.ttt_lr=.005
Hyperparameters.ttt_epochs=3
Hyperparameters.ttt_momentum=.9
Hyperparameters.gptq_reserve_seconds=12.
Hyperparameters.gptq_calibration_batches=64
Hyperparameters.matrix_bits=6
Hyperparameters.embed_bits=8
...
class Rotary(nn.Module):
	def __init__(self,dim,base=1e4,train_seq_len=1024,rope_dims=0):
		...
		inv_freq=1./base**(torch.arange(0,self.rope_dims,2,dtype=torch.float32)/self.rope_dims);self.register_buffer('inv_freq',inv_freq,persistent=False);self._seq_len_cached=0;self._cos_cached=None;self._sin_cached=None
		if bool(int(os.environ.get('ROTARY_PRECOMPUTE','0'))):self._precompute(train_seq_len,'cuda')
	def _precompute(self,seq_len,device):
		rd=self.rope_dims;inv_freq=self.inv_freq.to(device);t=torch.arange(seq_len,device=device,dtype=inv_freq.dtype);freqs=torch.outer(t,inv_freq);self._cos_cached=freqs.cos()[None,:,None,:].contiguous();self._sin_cached=freqs.sin()[None,:,None,:].contiguous();self._seq_len_cached=seq_len
	def forward(self,seq_len,device,dtype):
		if self._cos_cached is None or self._sin_cached is None or self._seq_len_cached<seq_len or self._cos_cached.device!=device:
			...
			t=torch.arange(seq_len,device=device,dtype=inv_freq.dtype);freqs=torch.outer(t,inv_freq);self._cos_cached=freqs.cos()[None,:,None,:].contiguous();self._sin_cached=freqs.sin()[None,:,None,:].contiguous();self._seq_len_cached=seq_len
		c=self._cos_cached[:,:seq_len];s=self._sin_cached[:,:seq_len];return c.to(dtype=dtype),s.to(dtype=dtype)
...
class CausalSelfAttention(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len):
		...
		self.num_heads=num_heads;self.num_kv_heads=num_kv_heads;self.head_dim=dim//num_heads;self.q_dim=dim;self.kv_dim=self.num_kv_heads*self.head_dim
		if self.head_dim%2!=0:raise ValueError('head_dim must be even for RoPE')
		self._pack_qkv=bool(int(os.environ.get('PACK_QKV','0')))
		if self._pack_qkv:self.c_qkv=CastedLinear(dim,self.q_dim+2*self.kv_dim,bias=False)
		else:self.c_q=CastedLinear(dim,dim,bias=False);self.c_k=CastedLinear(dim,self.kv_dim,bias=False);self.c_v=CastedLinear(dim,self.kv_dim,bias=False)
		self.proj=CastedLinear(dim,dim,bias=False);self.proj._zero_init=True;self.q_gain=nn.Parameter(torch.full((num_heads,),qk_gain_init,dtype=torch.float32));self.rope_dims=0;self.rotary=Rotary(self.head_dim,base=rope_base,train_seq_len=train_seq_len);self.use_xsa=False
	...
	def forward(self,x):
		...
		if self._pack_qkv:
			qkv=self.c_qkv(x);q,k,v=qkv.split([self.q_dim,self.kv_dim,self.kv_dim],dim=-1);q=q.reshape(bsz,seqlen,self.num_heads,self.head_dim);k=k.reshape(bsz,seqlen,self.num_kv_heads,self.head_dim);v=v.reshape(bsz,seqlen,self.num_kv_heads,self.head_dim)
...
def train_model(h,device,val_data):
	base_model=GPT(h).to(device).bfloat16();restore_fp32_params(base_model);_compile_kw=dict(dynamic=False,fullgraph=True)
	if h.compile_mode!='default':_compile_kw['mode']=h.compile_mode
	compiled_model=torch.compile(base_model,**_compile_kw);log(f"torch.compile: {_compile_kw}")
	...
	def lr_mul(frac):
		...
		return 1.
	_use_cudagraphs=h.compile_mode=='reduce-overhead'
	def step_fn(step,lr_scale):
		...
		for micro_step in range(h.grad_accum_steps):
			...
			if _use_cudagraphs:torch.compiler.cudagraph_mark_step_begin()
...
def main():
	...
	if bool(int(os.environ.get('INDUCTOR_EPILOGUE_FIRST','0'))):torch._inductor.config.epilogue_fusion_first=True
	if bool(int(os.environ.get('INDUCTOR_COORD_DESCENT','0'))):torch._inductor.config.coordinate_descent_tuning=True
