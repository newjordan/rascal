...
Hyperparameters.run_id='claude_partial_spinquant_sp8192_8x_seed1337'
...
def install_spinquant_eval_hooks(eval_model,h):
	# Eval-only: install register_forward_pre_hook on rotated CastedLinear modules
	# of eval_model. Hook left-multiplies input by R so F.linear(xR, WR) == F.linear(x, W).
	# Hooks live on eval_model instances only; base_model has no hooks installed.
