...
def prequant_ttt(h,device,val_data,base_model,batch_seqs=32):
	rank=h.rank;world_size=h.world_size;seq_len=h.eval_seq_len;total_tokens=val_data.val_tokens.numel()-1;n_epochs=h.prequant_ttt_epochs
	if n_epochs<=0:return
	for p in base_model.parameters():p.requires_grad_(True)
	if h.prequant_ttt_freeze_blocks>0:
		for _bi in range(min(h.prequant_ttt_freeze_blocks,len(base_model.blocks))):
			for _p in base_model.blocks[_bi].parameters():_p.requires_grad_(False)
	ttt_params=[p for p in base_model.parameters()if p.requires_grad]
	if h.prequant_ttt_optimizer.lower()=='adamw':optimizer=torch.optim.AdamW(ttt_params,lr=h.prequant_ttt_lr,weight_decay=h.prequant_ttt_wd,betas=(h.beta1,h.beta2),eps=h.adam_eps)
	elif h.prequant_ttt_optimizer.lower()=='sgd':optimizer=torch.optim.SGD(ttt_params,lr=h.prequant_ttt_lr,momentum=h.ttt_momentum,weight_decay=h.prequant_ttt_wd)
	else:raise ValueError(f"Unknown prequant_ttt_optimizer: {h.prequant_ttt_optimizer!r}")
	chunk_seqs=total_tokens//seq_len
	...
	log(f"prequant_ttt:start epochs={n_epochs} lr={h.prequant_ttt_lr} optimizer={h.prequant_ttt_optimizer} freeze_blocks={h.prequant_ttt_freeze_blocks} my_seqs={my_chunk_seqs}");base_model.train();t0=time.perf_counter()
	for ep in range(n_epochs):
		cos_lr=h.prequant_ttt_lr*.5*(1.+math.cos(math.pi*ep/max(n_epochs,1)))
		for pg in optimizer.param_groups:pg['lr']=cos_lr
		ep_loss=torch.zeros((),device=device,dtype=torch.float64);ep_tokens=torch.zeros((),device=device,dtype=torch.float64)
		for bs in range(0,my_chunk_seqs,batch_seqs):
			be=min(bs+batch_seqs,my_chunk_seqs);start_tok=(my_seq_s+bs)*seq_len;end_tok=(my_seq_s+be)*seq_len+1
			...
			if h.prequant_ttt_grad_clip>0:torch.nn.utils.clip_grad_norm_(ttt_params,h.prequant_ttt_grad_clip)
			optimizer.step();ep_loss+=loss.detach().to(torch.float64)*float(y.numel());ep_tokens+=float(y.numel())
		if dist.is_available()and dist.is_initialized():dist.all_reduce(ep_loss,op=dist.ReduceOp.SUM);dist.all_reduce(ep_tokens,op=dist.ReduceOp.SUM)
		ep_avg=(ep_loss/ep_tokens.clamp(min=1.)).item();log(f"prequant_ttt:epoch={ep+1}/{n_epochs} lr={cos_lr:.6f} train_loss={ep_avg:.4f} elapsed={time.perf_counter()-t0:.1f}s")
	for p in base_model.parameters():p.requires_grad_(True)
	base_model.eval()
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	log(f"ttt:start chunks={num_chunks} ttt_lr={h.ttt_lr} ttt_epochs={h.ttt_epochs} freeze_first_n_blocks={h.ttt_freeze_first_n_blocks}");compiled_logits=torch.compile(base_model.forward_logits,dynamic=False,fullgraph=True);loss_sum=torch.zeros((),device=device,dtype=torch.float64);token_count=torch.zeros((),device=device,dtype=torch.float64);byte_count=torch.zeros((),device=device,dtype=torch.float64)
	for p in base_model.parameters():p.requires_grad_(True)
	if h.ttt_freeze_first_n_blocks>0:
		for _bi in range(min(h.ttt_freeze_first_n_blocks,len(base_model.blocks))):
			for _p in base_model.blocks[_bi].parameters():_p.requires_grad_(False)
	ttt_params=[p for p in base_model.parameters()if p.requires_grad]
...
def train_and_eval(h,device):
	...
	if h.prequant_ttt_enabled:
		del compiled_model;torch._dynamo.reset();torch.cuda.empty_cache();prequant_ttt(h,device,val_data,base_model);torch._dynamo.reset();torch.cuda.empty_cache();compiled_model=torch.compile(base_model,dynamic=False,fullgraph=True);timed_eval('post-prequant-ttt',eval_val,h,device,val_data,compiled_model)
