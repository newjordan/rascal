# PR1855 SP10240 CaseOps Repro 10L MLP4 Late050

Run label: `new_experiment`, `standard_8x`.

This takes the closest over-cap SP10240 CaseOps MLP4 PR1855-port body down one
layer. The parent 11L MLP4 run had strong neural quality but exceeded the
16MB cap, so this spends the byte budget by removing one layer instead of
cutting MLP width. This variant moves loop activation to `0.50` because the
6x mechanics lane showed MLP4 late050 as the best post-EMA neural signal.
...
- PR1855 body adjusted: `num_layers=10`, `model_dim=512`, `mlp_mult=4.0`, `parallel_start_layer=7`
...
```bash
cd /workspace/sota_rascal/legs/2026-04-30_pr1855_sp10240_caseops_repro_10l_mlp4_late050_8x
./launch_8x.sh
tail -f logs/pr1855_sp10240_caseops_repro_10l_mlp4_late050_8x_seed42.txt
```
