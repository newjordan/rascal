...
Hyperparameters.source_body=SOURCE_BODY
Hyperparameters.source_body_sha256=SOURCE_BODY_SHA256
Hyperparameters.source_condition_artifact=SOURCE_CONDITION_ARTIFACT
Hyperparameters.source_condition_sha256=SOURCE_CONDITION_SHA256
Hyperparameters.source_submission_artifact=SOURCE_SUBMISSION_ARTIFACT
Hyperparameters.source_submission_sha256=SOURCE_SUBMISSION_SHA256
...
Hyperparameters.seed=444
Hyperparameters.run_id='mikey_sp10240_full124_loop2_late050_10l_mlp4_ppmbyte_8x_seed444'
Hyperparameters.max_wallclock_seconds=float(BUILD_SECONDS)
Hyperparameters.vocab_size=10240
Hyperparameters.num_layers=10
Hyperparameters.xsa_last_n=10
Hyperparameters.mlp_mult=4.
Hyperparameters.qk_gain_init=5.25
Hyperparameters.warmdown_frac=.72
Hyperparameters.train_log_every=500
Hyperparameters.val_loss_every=4000
Hyperparameters.sliding_window_enabled=True
Hyperparameters.num_loops=2
Hyperparameters.loop_start=3
Hyperparameters.loop_end=5
Hyperparameters.enable_looping_at=.50
Hyperparameters.parallel_residual_start=7
Hyperparameters.adam_wd=.02
Hyperparameters.muon_wd=.095
Hyperparameters.embed_wd=.085
Hyperparameters.ema_decay=.9965
Hyperparameters.matrix_clip_sigmas=13.05
Hyperparameters.ttt_enabled=True
Hyperparameters.ttt_lr=.005
Hyperparameters.ttt_epochs=3
Hyperparameters.ttt_momentum=.9
Hyperparameters.datasets_dir=os.path.join(Hyperparameters.data_dir,'datasets','fineweb10B_sp10240')
...
Hyperparameters.tokenizer_path=os.path.join(Hyperparameters.data_dir,'tokenizers',f"fineweb_{Hyperparameters.vocab_size}_bpe.model")
Hyperparameters.logfile=f"logs/{Hyperparameters.run_id}.txt"
Hyperparameters.ppm_enabled=True
Hyperparameters.ppm_mix_enabled=True
Hyperparameters.ppm_native_enabled=False
Hyperparameters.ppm_order=4
Hyperparameters.ppm_lambda_hi=.9
Hyperparameters.ppm_lambda_lo=.05
Hyperparameters.ppm_conf_threshold=.9
Hyperparameters.ppm_log_cache_size=16384
Hyperparameters.lambda_schedule='binary'
Hyperparameters.lambda_min=.05
Hyperparameters.lambda_max=.9
Hyperparameters.lambda_temp=.05
Hyperparameters.lambda_mid=.85
Hyperparameters.lambda_fixed=.5
...
class ShuffledSequenceLoader:
	def __init__(self,h,device):
		...
		self.prefetch_enabled=getattr(h,'data_prefetch',False);self._pf_thread=None;self._pf_queue=None;self._pf_stop=None;self._pf_args=None
	...
	def _pf_worker(self):
		try:
			while not self._pf_stop.is_set():
				x,y=self._build_batch_cpu(*self._pf_args);self._pf_queue.put((x,y))
		except BaseException as e:self._pf_queue.put(e)
	def _start_prefetch(self,global_tokens,grad_accum_steps):
		self._pf_args=(global_tokens,grad_accum_steps);self._pf_queue=queue.Queue(maxsize=2);self._pf_stop=threading.Event();self._pf_thread=threading.Thread(target=self._pf_worker,daemon=True);self._pf_thread.start()
	def stop_prefetch(self):
		if self._pf_thread is None:return
		self._pf_stop.set()
		try:
			while True:self._pf_queue.get_nowait()
		except queue.Empty:pass
		self._pf_thread=None
	def next_batch(self,global_tokens,grad_accum_steps):
		if self.prefetch_enabled:
			if self._pf_thread is None:self._start_prefetch(global_tokens,grad_accum_steps)
			item=self._pf_queue.get()
			if isinstance(item,BaseException):raise item
			x,y=item
		else:x,y=self._build_batch_cpu(global_tokens,grad_accum_steps)
...
class Optimizers:
	...
	def step(self):
		...
		self.zero_grad_all()
class EMAManager:
	def __init__(self,base_model,decay,foreach):
		self.decay=decay;self.foreach=foreach;sd=base_model.state_dict();self.keys=list(sd.keys());self.state={k:sd[k].detach().float().clone()for k in self.keys}
		if foreach:
			self.groups={}
			for k in self.keys:
				dt=sd[k].dtype;g=self.groups.setdefault(dt,{'ema':[],'staging':[],'keys':[]});g['ema'].append(self.state[k]);g['keys'].append(k)
				if dt!=torch.float32:g['staging'].append(torch.empty_like(self.state[k]))
	@torch.no_grad()
	def step(self,base_model):
		sd=base_model.state_dict()
		if self.foreach:
			for(dt,g)in self.groups.items():
				base_list=[sd[k].detach()for k in g['keys']]
				if dt==torch.float32:torch._foreach_lerp_(g['ema'],base_list,1.-self.decay)
				else:torch._foreach_copy_(g['staging'],base_list);torch._foreach_lerp_(g['ema'],g['staging'],1.-self.decay)
		else:
			for k in self.keys:self.state[k].mul_(self.decay).add_(sd[k].detach().float(),alpha=1.-self.decay)
	def apply_to(self,base_model):cur=base_model.state_dict();avg={k:self.state[k].to(dtype=cur[k].dtype)for k in self.keys};base_model.load_state_dict(avg,strict=True)
...
def _ppm_mixture_bpb(tgt_np,lp_np,sp,O=4,schedule='binary',lambda_hi=.9,lambda_lo=.05,conf_threshold=.9,lambda_min=.05,lambda_max=.9,lambda_temp=.05,lambda_mid=.85,lambda_fixed=.5):
	V=sp.vocab_size();piece_bytes=[None]*V;piece_lens=np.zeros(V,dtype=np.int32)
	for i in range(V):p=sp.id_to_piece(i);b=(' '+p[1:]).encode()if p.startswith('▁')else p.encode();piece_bytes[i]=b;piece_lens[i]=len(b)
	per_tok_len=piece_lens[tgt_np];bs=b"".join(piece_bytes[int(t)]for t in tgt_np);N=len(bs)
	if N==0:return 0.
	rep_lp=np.repeat(lp_np.astype(np.float64),per_tok_len);rep_len=np.repeat(per_tok_len.astype(np.float64),per_tok_len);nlp=np.where(rep_len>0,rep_lp/rep_len,0.)
	tabs=[dict()for _ in range(O+1)];plp=np.empty(N,dtype=np.float64);cf=np.empty(N,dtype=np.float64);log_=math.log;h=b""
	for i in range(N):
		x=bs[i]
		if i==0:plp[i]=math.log(1/256);cf[i]=1/256
		else:
			esc=1.;pf=0.;cf_mx=0;cf_tot=256;cf_seen=False;lim=O if i>O else i
			for o in range(lim,-1,-1):
				k=h[-o:]if o else b"";e=tabs[o].get(k)
				if e is None:continue
				if not cf_seen:cf_mx=e[1];cf_tot=e[0];cf_seen=True
				tot=e[0];d=e[2];c=d.get(x,0)
				if c>0:pf=esc*(2*c-1)/(2*tot);break
				esc*=len(d)/(2*tot)
			else:pf=esc/256
			if pf<1e-20:pf=1e-20
			plp[i]=log_(pf);cf[i]=(cf_mx/cf_tot)if cf_seen else 1/256
		for o in range(O+1):
			k=h[-o:]if o else b"";e=tabs[o].get(k)
			if e is None:tabs[o][k]=[1,1,{x:1}]
			else:
				e[0]+=1;d=e[2];cnt=d.get(x,0)+1;d[x]=cnt
				if cnt>e[1]:e[1]=cnt
		h=(h+bytes([x]))[-O:]
	if schedule=='binary':lam=np.where(cf>conf_threshold,lambda_lo,lambda_hi)
	elif schedule=='fixed':lam=np.full_like(cf,lambda_fixed)
	else:
		t=max(lambda_temp,1e-09);z=(cf-lambda_mid)/t;z=np.clip(z,-50.,50.);sig=1./(1.+np.exp(-z));lam=lambda_min+(lambda_max-lambda_min)*sig;lam=np.clip(lam,lambda_min,lambda_max)
	pm=lam*np.exp(nlp)+(1-lam)*np.exp(plp);return float(-np.log2(np.maximum(pm,1e-300)).sum()/N)
def _ppm_mix_dispatch(h,val_data,lp_chunks,tgt_chunks,device,nn_val_loss,nn_val_bpb,token_count,byte_count):
	if not(getattr(h,'ppm_enabled',False)and getattr(h,'ppm_mix_enabled',False)):return nn_val_loss,nn_val_bpb
	lpl=torch.cat(lp_chunks)if lp_chunks else torch.zeros(0,dtype=torch.float32,device=device);tgl=torch.cat(tgt_chunks)if tgt_chunks else torch.zeros(0,dtype=torch.int32,device=device)
	if dist.is_available()and dist.is_initialized():
		sz=torch.tensor([lpl.numel()],device=device,dtype=torch.long);asz=[torch.zeros_like(sz)for _ in range(h.world_size)];dist.all_gather(asz,sz);sizes=[int(x.item())for x in asz];mx=max(sizes)if sizes else 0
		if mx>0 and lpl.numel()<mx:lpl=torch.cat([lpl,torch.zeros(mx-lpl.numel(),dtype=lpl.dtype,device=device)]);tgl=torch.cat([tgl,torch.zeros(mx-tgl.numel(),dtype=tgl.dtype,device=device)])
		if h.rank==0:
			gl=[torch.zeros(mx,dtype=lpl.dtype,device=device)for _ in range(h.world_size)];gt=[torch.zeros(mx,dtype=tgl.dtype,device=device)for _ in range(h.world_size)];dist.gather(lpl,gl,dst=0);dist.gather(tgl,gt,dst=0)
			lpa=torch.cat([gl[r][:sizes[r]]for r in range(h.world_size)]).cpu().numpy();tga=torch.cat([gt[r][:sizes[r]]for r in range(h.world_size)]).cpu().numpy()
		else:dist.gather(lpl,None,dst=0);dist.gather(tgl,None,dst=0);return nn_val_loss,nn_val_bpb
	else:lpa=lpl.cpu().numpy();tga=tgl.cpu().numpy()
	if h.rank!=0:return nn_val_loss,nn_val_bpb
	V=val_data.sp.vocab_size();piece_lens=np.zeros(V,dtype=np.int32)
	for i in range(V):p=val_data.sp.id_to_piece(i);piece_lens[i]=len((' '+p[1:]).encode()if p.startswith('▁')else p.encode())
	nb=int(piece_lens[tga].sum())
	if nb<=0 or tga.size==0:return nn_val_loss,nn_val_bpb
	mb=_ppm_mixture_bpb(tga,lpa,val_data.sp,O=h.ppm_order,schedule=h.lambda_schedule,lambda_hi=h.ppm_lambda_hi,lambda_lo=h.ppm_lambda_lo,conf_threshold=h.ppm_conf_threshold,lambda_min=h.lambda_min,lambda_max=h.lambda_max,lambda_temp=h.lambda_temp,lambda_mid=h.lambda_mid,lambda_fixed=h.lambda_fixed)
	nn_byte_bpb=(-lpa.sum()/math.log(2))/max(nb,1);log(f"[ppm_mix] schedule={h.lambda_schedule} order={h.ppm_order} tokens={len(tga)} bytes={nb} NN_byte={nn_byte_bpb:.5f} mix={mb:.5f} delta={mb-nn_byte_bpb:+.5f} NN_full={nn_val_bpb:.5f}")
	mix_loss=mb*math.log(2.)*(byte_count.item()/token_count.item()) if token_count.item()>0 else nn_val_loss
	return mix_loss,mb
...
def eval_val_sliding(h,device,val_data,base_model,batch_seqs=32):
	...
	nn_val_loss,nn_val_bpb=_loss_bpb(loss_sum,token_count,byte_count);val_loss,val_bpb=_ppm_mix_dispatch(h,val_data,lp_chunks,tgt_chunks,device,nn_val_loss,nn_val_bpb,token_count,byte_count);base_model.train();return val_loss,val_bpb
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	nn_val_loss,nn_val_bpb=_loss_bpb(loss_sum,token_count,byte_count);val_loss,val_bpb=_ppm_mix_dispatch(h,val_data,lp_chunks,tgt_chunks,device,nn_val_loss,nn_val_bpb,token_count,byte_count);base_model.eval();return val_loss,val_bpb
...
def train_model(h,device,val_data):
	...
	if h.warmup_steps>0:
		initial_model_state={name:(tensor.detach().clone()if h.gpu_warmup_snapshot else tensor.detach().cpu().clone())for(name,tensor)in base_model.state_dict().items()};initial_optimizer_states=[copy.deepcopy(opt.state_dict())for opt in optimizers];model.train()
		...
		train_loader.stop_prefetch();train_loader=ShuffledSequenceLoader(h,device)
	ema=EMAManager(base_model,h.ema_decay,h.ema_foreach);training_time_ms=.0;stop_after_step=None;torch.cuda.synchronize();t0=time.perf_counter();step=0
	while True:
		...
		train_loss=step_fn(step,scale);ema.step(base_model)
	...
	log(f"peak memory allocated: {torch.cuda.max_memory_allocated()//1024//1024} MiB reserved: {torch.cuda.max_memory_reserved()//1024//1024} MiB");log(f"ema:applying EMA weights (foreach={h.ema_foreach})");ema.apply_to(base_model);return base_model,compiled_model
