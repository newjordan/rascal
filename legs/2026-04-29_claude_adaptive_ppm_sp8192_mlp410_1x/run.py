...
Hyperparameters.data_dir='/workspace/Fartmagic/data/'
Hyperparameters.seed=1337
Hyperparameters.run_id='claude_adaptive_ppm_sp8192_mlp4_qk525_1x_seed1337'
...
Hyperparameters.ppm_native_enabled=True
Hyperparameters.ppm_order=4
Hyperparameters.lambda_schedule='adaptive'
...
def build_token_bytes_lut(sp,vocab_size):
	sp_vocab_size=int(sp.vocab_size());n=max(sp_vocab_size,vocab_size);out=[b'']*n
	for tid in range(sp_vocab_size):
		if sp.is_control(tid)or sp.is_unknown(tid)or sp.is_unused(tid):out[tid]=b'';continue
		if sp.is_byte(tid):
			s=sp.id_to_piece(tid)
			if len(s)==6 and s.startswith('<0x')and s.endswith('>'):out[tid]=bytes([int(s[3:5],16)])
			else:out[tid]=s.encode('utf-8')[:1]if s else b''
			continue
		piece=sp.id_to_piece(tid)
		if piece.startswith('▁'):out[tid]=(' '+piece[1:]).encode('utf-8')
		else:out[tid]=piece.encode('utf-8')
	return out
...
def _ppm_mixture_bpb(tgt_np,lp_np,sp,O=4,schedule='adaptive',lambda_hi=.9,lambda_lo=.05,conf_threshold=.9,lambda_min=.05,lambda_max=.9,lambda_temp=.05,lambda_mid=.85,lambda_fixed=.5):
	...
	tabs=[dict()for _ in range(O+1)];plp=np.empty(N,dtype=np.float64);cf=np.empty(N,dtype=np.float64);LN256=math.log(1/256);log_=math.log;h=b""
	for i in range(N):
		x=bs[i]
		if i==0:plp[i]=LN256;cf[i]=1/256
		else:
			esc=1.;pf=0.;cf_mx=0;cf_tot=256;cf_seen=False;lim=O if i>O else i
			# Strict-legal gate: cf is frozen at the deepest seen context BEFORE the d.get(x,...) lookup below.
...
def _ppm_mix_dispatch(h,val_data,lp_chunks,tgt_chunks,device,nn_val_bpb):
	if not(h.ppm_enabled and h.ppm_mix_enabled):return nn_val_bpb
	...
	if dist.is_available()and dist.is_initialized():
		...
		else:dist.gather(lpl,None,dst=0);dist.gather(tgl,None,dst=0);return nn_val_bpb
	else:lpa=lpl.cpu().numpy();tga=tgl.cpu().numpy()
	if h.rank!=0:return nn_val_bpb
	V=val_data.sp.vocab_size();_pl=np.zeros(V,dtype=np.int32)
	for _i in range(V):_p=val_data.sp.id_to_piece(_i);_pl[_i]=len((' '+_p[1:]).encode()if _p.startswith('▁')else _p.encode())
	nb=int(_pl[tga].sum())
	if nb<=0 or tga.size==0:return nn_val_bpb
	...
	nn_byte_bpb=(-lpa.sum()/math.log(2))/max(nb,1);log(f"[ppm_mix] schedule={h.lambda_schedule} order={h.ppm_order} tokens={len(tga)} bytes={nb} NN_byte={nn_byte_bpb:.5f} mix={mb:.5f} delta={mb-nn_byte_bpb:+.5f} NN_full={nn_val_bpb:.5f}");return mb
...
def eval_val_sliding(h,device,val_data,base_model,batch_seqs=32):
	...
	nn_val_loss,nn_val_bpb=_loss_bpb(loss_sum,token_count,byte_count);val_bpb_out=_ppm_mix_dispatch(h,val_data,lp_chunks,tgt_chunks,device,nn_val_bpb);base_model.train();return nn_val_loss,val_bpb_out
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	nn_val_loss,nn_val_bpb=_loss_bpb(loss_sum,token_count,byte_count);val_bpb_out=_ppm_mix_dispatch(h,val_data,lp_chunks,tgt_chunks,device,nn_val_bpb);base_model.eval();return nn_val_loss,val_bpb_out
