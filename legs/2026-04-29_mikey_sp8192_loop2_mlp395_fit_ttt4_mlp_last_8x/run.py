...
TEST_ID='2026-04-29_mikey_sp8192_loop2_mlp395_fit_ttt4_mlp_last_8x'
...
PARENT_RUN='2026-04-29_mikey_sp8192_loop2_mlp395_fit_ttt4_8x'
HYPOTHESIS='8k legal-size focused-TTT swing: keep SP8192 MLP3.95 loop2 fit condition unchanged, run TTT epochs 1-3 on all parameters, then spend epoch 4 only on MLP matrix weights to target the dominant quantized matrix damage without changing bytes.'
...
Hyperparameters.run_id='mikey_sp8192_loop2_mlp395_fit_ttt4_mlp_last_8x_seed444'
...
Hyperparameters.ttt_epochs=4
Hyperparameters.ttt_focus_last_epoch='mlp'
...
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	...
	log(f"ttt:start chunks={num_chunks} ttt_lr={h.ttt_lr} ttt_epochs={h.ttt_epochs}");compiled_logits=torch.compile(base_model.forward_logits,dynamic=False,fullgraph=True);loss_sum=torch.zeros((),device=device,dtype=torch.float64);token_count=torch.zeros((),device=device,dtype=torch.float64);byte_count=torch.zeros((),device=device,dtype=torch.float64);named_ttt_params=list(base_model.named_parameters());ttt_params=[p for _,p in named_ttt_params];focus_mode=getattr(h,'ttt_focus_last_epoch','');ttt_focus_params=[p for n,p in named_ttt_params if focus_mode=='mlp' and '.mlp.' in n and n.endswith('.weight')]
	if focus_mode:
		if not ttt_focus_params:raise ValueError(f"TTT focus mode {focus_mode!r} selected no parameters")
		log(f"ttt:focus_last_epoch={focus_mode} focus_params={sum(p.numel()for p in ttt_focus_params)}")
		...
		if not is_last_chunk and h.ttt_epochs>0:
			...
			if chunk_seqs>0:
				...
				for _ep in range(h.ttt_epochs):
					active_params=ttt_focus_params if focus_mode and _ep==h.ttt_epochs-1 else ttt_params
					active_ids={id(p)for p in active_params}
					for p in ttt_params:p.requires_grad_(id(p)in active_ids)
