...
TEST_ID='2026-04-29_mikey_sp5200_mlp4_mixedquant_post_8x'
TEST_DATE='2026-04-29'
RUN_LABEL='posttrain_quant_swing_8x'
...
HYPOTHESIS='Post-train SP5200 mixed-quant swing from the clean MLP4 checkpoint: keep int8 embeddings, upgrade all attention matrices to int8, upgrade loop MLP L3-5 matrices to int7, keep other MLP matrices int6, then run sliding+TTT.'
...
Hyperparameters.run_id='mikey_sp5200_mlp4_mixedquant_post_8x_seed1337'
...
Hyperparameters.posttrain_checkpoint_path='/workspace/sota_rascal/legs/2026-04-29_mikey_pr1493parity_sp5200_loop2_mlp4_8x/final_model.pt'
Hyperparameters.mixed_quant_policy='attn8_loopmlp7'
Hyperparameters.attn_bits=8
Hyperparameters.loop_mlp_bits=7
Hyperparameters.default_mlp_bits=6
...
def gptq_quantize_weight(w,H,clip_sigmas=3.,clip_range=63,block_size=128):
	...
	return Q[:,invperm],s
def quant_bits_for_name(name,h):
	if'tok_emb'in name:return h.embed_bits
	policy=getattr(h,'mixed_quant_policy','baseline')
	if policy=='attn8_loopmlp7':
		match=re.search(r'blocks\.(\d+)\.',name);layer=int(match.group(1))if match else -1
		if'.attn.'in name:return getattr(h,'attn_bits',8)
		if'.mlp.'in name and h.loop_start<=layer<=h.loop_end:return getattr(h,'loop_mlp_bits',7)
		if'.mlp.'in name:return getattr(h,'default_mlp_bits',h.matrix_bits)
...
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	for(name,tensor)in state_dict.items():
		...
		cs=h.embed_clip_sigmas if'tok_emb'in name else h.matrix_clip_sigmas;bits=quant_bits_for_name(name,h);q,s=gptq_quantize_weight(t,hessians[name],clip_sigmas=cs,clip_range=2**(bits-1)-1);result[name+'.q']=q;result[name+'.scale']=s;meta[name]=f"gptq (int{bits})"
...
def posttrain_quant_eval(h,device):
	random.seed(h.seed);np.random.seed(h.seed);torch.manual_seed(h.seed);torch.cuda.manual_seed_all(h.seed);val_data=ValidationData(h,device);checkpoint=Path(h.posttrain_checkpoint_path)
	if not checkpoint.exists():raise FileNotFoundError(f"posttrain checkpoint not found: {checkpoint}")
	log(f"train_shards: {len(list(Path(h.datasets_dir).resolve().glob('fineweb_train_*.bin')))}");log(f"val_tokens: {val_data.val_tokens.numel()-1}");log(f"posttrain_checkpoint: {checkpoint}")
	base_model=GPT(h).to(device).bfloat16();restore_fp32_params(base_model);state=torch.load(checkpoint,map_location='cpu');base_model.load_state_dict(state,strict=True)
	if h.num_loops>0:base_model.looping_active=True;log(f"layer_loop:forced_from_checkpoint encoder:{base_model.encoder_indices} decoder:{base_model.decoder_indices}")
	log(f"model_params:{sum(p.numel()for p in base_model.parameters())}");compiled_model=torch.compile(base_model,dynamic=False,fullgraph=True);timed_eval('posttrain_checkpoint',eval_val,h,device,val_data,compiled_model);del compiled_model;torch._dynamo.reset();serialize(h,base_model,Path(__file__).read_text(encoding='utf-8'))
...
def train_and_eval(h,device):
	if getattr(h,'posttrain_checkpoint_path',''):return posttrain_quant_eval(h,device)
