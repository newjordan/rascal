...
Hyperparameters.run_id='mikey_sp8192_freerun_fork_mlp4_qk525_8x_seed1337'
...
Hyperparameters.freerun_enabled=True
Hyperparameters.freerun_weight=.1
Hyperparameters.freerun_pos_shift=1
Hyperparameters.freerun_collapse_penalty=.05
Hyperparameters.freerun_dropout_p=.0
...
class Rotary(nn.Module):
	...
	def forward_shifted(self,seq_len,device,dtype,pos_shift):
		# Build cos/sin for positions [pos_shift..pos_shift+seq_len-1] without disturbing the cache.
		rd=self.rope_dims
		if seq_len+pos_shift>self.train_seq_len:scale=(seq_len+pos_shift)/self.train_seq_len;new_base=self.base*scale**(rd/(rd-2));inv_freq=1./new_base**(torch.arange(0,rd,2,dtype=torch.float32,device=device)/rd)
		else:inv_freq=self.inv_freq.to(device)
		t=torch.arange(pos_shift,pos_shift+seq_len,device=device,dtype=inv_freq.dtype);freqs=torch.outer(t,inv_freq);cos=freqs.cos()[None,:,None,:];sin=freqs.sin()[None,:,None,:];return cos.to(dtype=dtype),sin.to(dtype=dtype)
...
class CausalSelfAttention(nn.Module):
	...
	def forward(self,x,pos_shift=0):
		bsz,seqlen,dim=x.shape;q=self.c_q(x).reshape(bsz,seqlen,self.num_heads,self.head_dim);k=self.c_k(x).reshape(bsz,seqlen,self.num_kv_heads,self.head_dim);v=self.c_v(x).reshape(bsz,seqlen,self.num_kv_heads,self.head_dim);q=F.rms_norm(q,(q.size(-1),));k=F.rms_norm(k,(k.size(-1),))
		if pos_shift!=0:cos,sin=self.rotary.forward_shifted(seqlen,x.device,q.dtype,pos_shift)
		else:cos,sin=self.rotary(seqlen,x.device,q.dtype)
		q=apply_rotary_emb(q,cos,sin,self.rope_dims);k=apply_rotary_emb(k,cos,sin,self.rope_dims);q=q*self.q_gain.to(dtype=q.dtype)[None,None,:,None];y=flash_attn_3_func(q,k,v,causal=True)
...
class Block(nn.Module):
	...
	def forward(self,x,x0,pos_shift=0):
		mix=self.resid_mix.to(dtype=x.dtype);x_in=mix[0][None,None,:]*x+mix[1][None,None,:]*x0;attn_out=self.attn(self.attn_norm(x_in)*self.ln_scale_factor,pos_shift=pos_shift)
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.looping_active=False;self.loop_start=h.loop_start;self.loop_end=h.loop_end;self.num_loops=h.num_loops
		if h.num_loops>0:
		...
		self.freerun_enabled=h.freerun_enabled;self.freerun_weight=h.freerun_weight;self.freerun_pos_shift=h.freerun_pos_shift;self.freerun_collapse_penalty=h.freerun_collapse_penalty;self.freerun_dropout_p=h.freerun_dropout_p
		self._init_weights()
	...
	def forward_logits(self,input_ids):
		# Branch A only. Used by all eval paths and by training when freerun is disabled.
		...
		x=self.final_norm(x);return self._project_to_logits(x)
	def forward_logits_branched(self,input_ids):
		# Branch A logits, plus Branch B logits via a free-running predictive loop pass on the same shared weights.
		# Causality: Branch B at position t only consumes encoder state H at positions <= t (causal attention preserved
		# by flash_attn causal=True). The loop pass uses a RoPE position shift of +pos_shift; no token-level shift.
		x=self.tok_emb(input_ids);x=F.rms_norm(x,(x.size(-1),))
		if self.embed_proj is not None:x=self.embed_proj(x)
		x0=x;skips=[];enc_iter=self.encoder_indices if self.looping_active else range(self.num_encoder_layers);dec_iter=self.decoder_indices if self.looping_active else range(self.num_encoder_layers,self.num_encoder_layers+self.num_decoder_layers)
		for i in enc_iter:x=self.blocks[i](x,x0);skips.append(x)
		# Branch A: standard decoder over encoder output x.
		dec_skips_a=list(skips);x_a=x
		for(skip_idx,i)in enumerate(dec_iter):
			if skip_idx<self.num_skip_weights and dec_skips_a:
				scaled_skip=self.skip_weights[skip_idx].to(dtype=x_a.dtype)[None,None,:]*dec_skips_a.pop()
				if self.skip_gates is not None:g=torch.sigmoid(self.skip_gates[skip_idx].to(dtype=x_a.dtype))[None,None,:];x_a=torch.lerp(scaled_skip,x_a,g)
				else:x_a=x_a+scaled_skip
			x_a=self.blocks[i](x_a,x0)
		x_a=self.final_norm(x_a);logits_a=self._project_to_logits(x_a)
		# Branch B: predicted_H_{t+1}* = one extra free-running loop pass over [loop_start..loop_end] with shared
		# weights and a positive RoPE position shift, starting from the encoder output H. x0 acts as a
		# position-anchored residual; we shift it identically by reusing the same x0 (no new tensor needed).
		# Causality in Branch B: each block here is causal self-attention; the rotary shift only modifies
		# relative positions for queries vs keys uniformly, leaving the causal mask intact.
		h_t=x;x_b=x
		for i in range(self.loop_start,self.loop_end+1):x_b=self.blocks[i](x_b,x0,pos_shift=self.freerun_pos_shift)
		# Optional positional dropout: zero out a fraction of predicted positions so Branch B's decoder must
		# learn from partial signal (regularizer against trivial collapse).
		if self.freerun_dropout_p>0.:
			keep=torch.empty((x_b.size(0),x_b.size(1),1),device=x_b.device,dtype=x_b.dtype).bernoulli_(1.-self.freerun_dropout_p);x_b=x_b*keep
		# Branch B decoder reuses the encoder skips (info <= t) — same causal envelope as Branch A.
		dec_skips_b=list(skips);x_b_dec=x_b
		for(skip_idx,i)in enumerate(dec_iter):
			if skip_idx<self.num_skip_weights and dec_skips_b:
				scaled_skip=self.skip_weights[skip_idx].to(dtype=x_b_dec.dtype)[None,None,:]*dec_skips_b.pop()
				if self.skip_gates is not None:g=torch.sigmoid(self.skip_gates[skip_idx].to(dtype=x_b_dec.dtype))[None,None,:];x_b_dec=torch.lerp(scaled_skip,x_b_dec,g)
				else:x_b_dec=x_b_dec+scaled_skip
			x_b_dec=self.blocks[i](x_b_dec,x0)
		x_b_dec=self.final_norm(x_b_dec);logits_b=self._project_to_logits(x_b_dec);return logits_a,logits_b,h_t,x_b
	def forward(self,input_ids,target_ids):
		if not(self.training and self.freerun_enabled and self.freerun_weight>0.):
		...
		logits_a,logits_b,h_t,h_pred=self.forward_logits_branched(input_ids);ce_a=F.cross_entropy(logits_a.reshape(-1,logits_a.size(-1)).float(),target_ids.reshape(-1),reduction='mean')
		# Branch B logits at position t predict the token at original position t+2 (since target_ids is already
		# shifted by +1 and Branch B carries an additional predictive shift of +1). Compare logits_b[:, :-1] to
		# target_ids[:, 1:]. Drop the final position (no valid target) — does not change asymptotic complexity.
		tgt_b=target_ids[:,1:].contiguous();logits_b_used=logits_b[:,:-1,:].contiguous();ce_b=F.cross_entropy(logits_b_used.reshape(-1,logits_b_used.size(-1)).float(),tgt_b.reshape(-1),reduction='mean')
		# Collapse penalty: discourage h_pred from being trivially close to h_t (degenerate identity solution).
		# Add cosine similarity to the loss so the optimizer pushes h_pred away from h_t.
		if self.freerun_collapse_penalty>0.:cos_sim=F.cosine_similarity(h_pred.float(),h_t.float(),dim=-1).mean();collapse_term=self.freerun_collapse_penalty*cos_sim
		else:collapse_term=h_pred.sum()*0.
		return ce_a+self.freerun_weight*ce_b+collapse_term
...
def eval_val_ttt(h,device,val_data,base_model,batch_seqs=32):
	# TTT: Branch A only. base_model.training is True during the SGD phase, but we explicitly disable freerun
	# below so train-time forward goes through forward_logits (Branch A) regardless. forward() uses the same code
	# path as eval when freerun_enabled is False or freerun_weight is 0.
	prior_freerun=base_model.freerun_enabled;base_model.freerun_enabled=False
	...
	base_model.eval();base_model.freerun_enabled=prior_freerun;return _loss_bpb(loss_sum,token_count,byte_count)
...
def train_and_eval(h,device):
	...
	eval_model.freerun_enabled=False
	...
	if h.ttt_enabled and h.sliding_window_enabled:
		...
		ttt_model.freerun_enabled=False
