...
os.environ.setdefault("RUN_ID", "rascal_sp4096_loop_1x_smoke_seed444_" + time.strftime("%Y%m%d_%H%M%S"))

TEST_ID = "2026-04-29_rascal_sp4096_loop_1x_smoke"
RUN_LABEL = "mechanics_proxy_1x_smoke"
SOURCE_SCRIPT_SHA256 = "b93227b9f394b81fa77c412e0ecc394f17fb9effb250b96fe77903b9e1687d39"
SOURCE_SCRIPT_PATH = "quarantine/2026-04-28_tokenizer_provenance/candidate_submissions/Raphe_1.1/train_gpt_8xgpu.py"
EVIDENCE_GATE_PATH = "notes/2026-04-29_kernel_loop_mining_brief.md"
EVIDENCE_GATE_SHA256 = "e3047ae3c94455be41c601fb023cc5e855a94e4e54c6fe618a6606efbae64689"
AUDITED_TOKENIZER_MODEL_SHA256 = "6b0337698df13acdb58e5b05446ef26d9d7e4ca5545dc5e13ad5815a7ec904e0"
AUDITED_TOKENIZER_VOCAB_SHA256 = "925129ee4771d6f0cdb83d1284ca023cfb37360fb7b759c966cbce911ab44ad9"
...
def require_sha256(path: str | Path, expected: str, label: str) -> str:
    ...
    got = sha256_file(p)
    if got != expected:
        raise RuntimeError(f"{label} sha256 mismatch: got {got}, expected {expected}, path={p}")
    return got
...
class Hyperparameters:
    data_path = os.environ.get("DATA_PATH", "/workspace/data/datasets/fineweb10B_sp4096")
    train_files = os.path.join(data_path, "fineweb_train_*.bin")
    val_files = os.path.join(data_path, "fineweb_val_*.bin")
    tokenizer_path = os.environ.get("TOKENIZER_PATH", "/workspace/data/tokenizers/fineweb_4096_bpe.model")
    ...
    max_wallclock_seconds = float(os.environ.get("MAX_WALLCLOCK_SECONDS", BUILD_SECONDS))
...
def leaky_relu_sq(x: Tensor, kernel_mode: str = "") -> Tensor:
    if kernel_mode:
        raise RuntimeError(f"{TEST_ID} is FA3/control only; MLP_KERNEL_MODE={kernel_mode!r} is not allowed")
    a = F.leaky_relu(x, negative_slope=0.5)
    return a.square()
...
def main() -> None:
    ...
    if world_size != 1:
        raise ValueError(
            f"{TEST_ID} requires WORLD_SIZE=1, got {world_size}. "
            "Launch with: torchrun --standalone --nproc_per_node=1 run.py"
        )
    ...
    log0(f"condition_id:{TEST_ID}")
    log0(f"run_label:{RUN_LABEL} source_script:{SOURCE_SCRIPT_PATH} source_script_sha256:{SOURCE_SCRIPT_SHA256}")
    log0(f"evidence_gate:{EVIDENCE_GATE_PATH} evidence_gate_sha256:{EVIDENCE_GATE_SHA256}")
    log0("changed_fields:Raphe1.1_10L->Rascal_loop_11L, NUM_LOOPS=2, LOOP_START=2, LOOP_END=4, gpus 8->1, world_size 8->1, grad_accum_steps 1->8, wallclock 600->300, iterations 20000->8, warmup 20->2, final_sliding_eval skipped")
    log0(f"condition:BUILD_SECONDS={BUILD_SECONDS} EVAL_SECONDS={EVAL_SECONDS} SIZE_CAP_BYTES={SIZE_CAP_BYTES}")
    log0("expected_metric:loop_mechanics_smoke comparator:Donnie_Raphe1.1_loop_topology final_sliding_window_exact_skipped")
    ...
    log0(f"condition:NUM_LAYERS={args.num_layers}")
    log0(f"condition:MODEL_DIM={args.model_dim}")
    log0(f"condition:MLP_MULT={args.mlp_mult}")
    log0(f"condition:SEED={args.seed}")
    log0(f"condition:ITERATIONS={args.iterations}")
    log0(f"condition:WARMUP_STEPS={args.warmup_steps}")
    log0(f"condition:VAL_LOSS_EVERY={args.val_loss_every}")
    ...
    log0(f"condition:SKIP_FINAL_EVAL={int(args.skip_final_eval)}")
    log0(f"condition:POST_EMA_DIAGNOSTIC={int(args.post_ema_diagnostic)}")
    ...
    tokenizer_sha = require_sha256(args.tokenizer_path, AUDITED_TOKENIZER_MODEL_SHA256, "SP4096 tokenizer model")
    vocab_path = str(Path(args.tokenizer_path).with_suffix(".vocab"))
    vocab_sha = require_sha256(vocab_path, AUDITED_TOKENIZER_VOCAB_SHA256, "SP4096 tokenizer vocab")
    log0(f"condition:SP4096_MODEL_SHA256={tokenizer_sha}")
    log0(f"condition:SP4096_VOCAB_SHA256={vocab_sha}")
    ...
    log0(f"fa3_available:{int(flash_attn_3_func is not None)} kernel_policy:FA3_control_no_custom_kernel compile_enabled={int(args.compile_enabled)} mlp_kernel={args.mlp_kernel_mode or 'eager'}")
    ...
    log0(f"eval_model.looping_active:{int(eval_model.looping_active)}")
    torch.cuda.synchronize()
    ...
    if args.skip_final_eval:
        log0("final_eval:skipped sliding by SKIP_FINAL_EVAL=1")
    else:
